...
function assertSemantic(bundle, label) {
  ...
  assert.ok(bundle.operations.some((operation) => operation.kind !== 'unknown'), `${label}: definite operation remains`);
}
function assertFailClosed(bundle, label) {
  assert.ok(bundle, label);
  assert.equal(bundle.completeness, 'partial', `${label}: malformed sysreg is partial`);
  assert.ok(bundle.operations.every((operation) => operation.kind === 'unknown'), `${label}: malformed sysreg emits no definite operation`);
}
