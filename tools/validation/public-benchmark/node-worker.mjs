...
export class ProductNodeWorker {
  constructor(url, options={}) {
    this.ready = false; this.queue = []; this.listeners = new Map(); this.closed = false;
    ...
    active.add(this);
    this.timer = setTimeout(() => { this.emit('error', {message:'worker-boot-timeout'}); this.terminate(); }, 15000);
  ...
  terminate(){this.closed=true;clearTimeout(this.timer);this.queue=[];active.delete(this);return this.thread.terminate();}
}
