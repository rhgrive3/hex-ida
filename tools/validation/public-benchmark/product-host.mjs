...
export async function openProduct(binaryPath) {
  ...
  if (sliceIndex < 0) return { unsupported: true, reason: 'ARM64_SLICE_UNAVAILABLE', info, sha };
  ...
  app.symbols = new SymbolIndex({ ...await backend.analyze(sliceIndex), regions:slice.regions });
  ...
  return { app, info, sha, sliceIndex, query:app.analysisQueries, close:async()=>{backend.dispose();runtime.close();await closeNodeWorkers();} };
}
