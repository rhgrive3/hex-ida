...
import { sourceManifest, sha256 } from '../performance-local/source-manifest.mjs';
...
const metadata = { deterministicTransforms: true, root, node: process.version, sourceManifest: sourceManifest(root), startedAt: new Date().toISOString() };
...
for (const phase8Optimize of [false, true]) {
  for (let index = 0; index < corpus.functions.length; index++) {
    ...
    const outcome = decompileEntry(entry, { index, deterministicTransforms: true, phase8Optimize });
