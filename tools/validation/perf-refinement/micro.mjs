...
if(!rootArg||!outArg)throw new Error('usage: node micro.mjs <repository-root> <output.json>');
...
const hashes=[],rows=[];let sink;
const digest=value=>createHash('sha256').update(JSON.stringify(value,(_k,v)=>typeof v==='bigint'?{$bigint:String(v)}:v instanceof Map?[...v]:v instanceof Set?[...v]:v)).digest('hex');
function bench(id,iterations,fn){
 for(let k=0;k<2;k++)for(let i=0;i<iterations;i++)sink=fn(i);
 const samples=[];
 for(let k=0;k<7;k++){const t=performance.now();for(let i=0;i<iterations;i++)sink=fn(i);samples.push((performance.now()-t)/iterations);}
 const output=fn(0),hash=digest(output);hashes.push([id,hash]);
 rows.push({id,iterations,samplesMs:samples,medianMs:[...samples].sort((a,b)=>a-b)[3],outputSha256:hash});
}
...
const result={schemaVersion:1,root,node:process.version,startedAt:new Date().toISOString(),rows,maxRssKiB:process.resourceUsage().maxRSS,
 outputSha256:digest(hashes),harnessSha256:createHash('sha256').update(fs.readFileSync(new URL(import.meta.url))).digest('hex'),
 note:'Fixed component workloads; medians per operation. No cross-task weighting or percentage averaging.'};
fs.mkdirSync(path.dirname(path.resolve(outArg)),{recursive:true});fs.writeFileSync(outArg,JSON.stringify(result,null,2)+'\n');
console.log(JSON.stringify(result.rows.map(r=>({id:r.id,ms:r.medianMs}))));
