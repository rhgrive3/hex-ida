...
/**
 * Records one P7-I checkpoint acceptance.
 *
 * The ledger is durable repository evidence rather than conversation state, so
 * a long autonomous run that spans merges and reloads can say exactly where it
 * reached and what proved it (EP-030). Each entry binds the exact head, the
 * changed-file inventory digest, the ownership-policy version, the corpus
 * manifest identity, and the verifier version — a generic "CI green" line is
 * explicitly not enough (§13.2).
 */

const ROOT = path.resolve(path.dirname(fileURLToPath(import.meta.url)), '../../..');
const LEDGER = path.join(ROOT, 'reports/phase7/checkpoints.json');
...
export function recordCheckpoint({
  ...
  const ownership = JSON.parse(fs.readFileSync(path.join(ROOT, 'tools/validation/phase-ownership/phase7.json'), 'utf8'));
...
if (isMain) {
  ...
  if (!id || !baseSha) {
    console.error('usage: node tools/validation/phase7/checkpoint.mjs --id P7-1 --base <sha> [--gates a,b] [--result accepted|blocking] [--reason text]');
    process.exitCode = 2;
  } else {
