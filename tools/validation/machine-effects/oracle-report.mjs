...
const EXTERNAL_EVIDENCE_STATUSES = Object.freeze(['available', 'unavailable', 'partial', 'unsupported']);
...
function unjustifiedExactCases(exactClaims, evidence) {
  // Claim-local evidence remains diagnostic until an integration-owned release
  // transaction supplies and validates the generated manifest. A component
  // checkout must never discover its own committed output and promote it.
...
function profileSummaries(corpus, results) {
  ...
  return ORACLE_PROFILE_INVENTORY.map((profile) => {
    ...
    const gaps = cases.flatMap((item) => {
      const result = resultsByCase.get(item.caseId);
      if (!result) return [{ status: 'not-integrated', caseId: item.caseId }];
      return PASS_STATUSES.includes(result.status) ? [] : [{ status: result.status, caseId: result.caseId }];
    });
...
function normalizeExternalEvidence(value) {
  ...
  return value.map((entry) => {
    ...
    const profileId = identity(entry.profileId, 'report-external-evidence-profile');
    const profile = ORACLE_PROFILE_INVENTORY.find((item) => item.profileId === profileId);
    if (!profile) fail('report-external-evidence-profile-invalid');
    if (seen.has(profileId)) fail('report-external-evidence-profile-duplicate', profileId);
    seen.add(profileId);
    const authorityId = assertIndependentText(entry.authorityId, 'report-external-evidence-authority');
    const toolchainIdentity = assertIndependentText(entry.toolchainIdentity, 'report-external-evidence-toolchain');
    if (authorityId !== profile.authorityId) fail('report-external-evidence-authority-mismatch', profileId);
    if (toolchainIdentity !== profile.toolchainIdentity) fail('report-external-evidence-toolchain-mismatch', profileId);
    const status = identity(entry.status, 'report-external-evidence-status');
    if (!EXTERNAL_EVIDENCE_STATUSES.includes(status)) fail('report-external-evidence-status-invalid', status);
    return {
      profileId,
      authorityId,
...
export function createOracleReport({
  ...
  const missingCases = normalizedCorpus.cases.filter((item) => !seenResultCases.has(item.caseId));
  counts['not-integrated'] += missingCases.length;
  ...
  const reportWithoutId = {
    ...
    passCount,
    totalCount: normalizedCorpus.cases.length,
    nonPassReasons: [
      ...normalizedResults.flatMap((result) => result.diagnostics.map((item) => ({ caseId: result.caseId, status: result.status, code: item.code }))),
      ...missingCases.map((item) => ({ caseId: item.caseId, status: 'not-integrated', code: 'result-missing' })),
    ],
    blockingCount: BLOCKING_STATUSES.reduce((sum, status) => sum + counts[status], 0),
