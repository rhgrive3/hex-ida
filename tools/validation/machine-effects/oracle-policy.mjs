...
export function assertIndependentText(value, code = 'independent-text-invalid') {
  return assertNonEmptyString(value, code);
}
...
export function assertDistinctOracleIdentity(identity, code = 'oracle-identity-invalid') {
  ...
  if (PRODUCTION_ORACLE_IDENTITIES.includes(normalized)) {
...
export function assertIndependentProvenance(provenance, {
  ...
  if (expected != null) {
    const expectedFields = ['authorityId', 'authorityRole', 'isaReference', 'referenceRevision', 'executionSource', 'sourceKind', 'toolchainIdentity'];
    for (const key of expectedFields) {
      if (normalized[key] !== expected[key]) throw new TypeError(`${code}:${key}-mismatch`);
    }
  }
  return normalized;
}
