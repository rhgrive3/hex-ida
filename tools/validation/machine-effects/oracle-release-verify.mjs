...
export function changedFilesWithinAllowlist(
  paths,
  ...
  const matchesRule = (file, rule) => rule.endsWith('/') ? file.startsWith(rule) : file === rule;
  const violations = paths.filter((file) => !isCanonicalRepositoryPath(file)
    || forbiddenPaths.some((rule) => matchesRule(file, rule))
    || !allowedPrefixes.some((rule) => matchesRule(file, rule)));
