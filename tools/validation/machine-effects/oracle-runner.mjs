...
function diagnostic(code, detail = null) {
  return typeof detail === 'string' ? { code, detail: boundedText(detail) } : { code };
}
...
function snapshotUntrustedData(value, context = { seen: new WeakSet(), nodes: 0 }, depth = 0) {
  if (depth >= SNAPSHOT_MAX_DEPTH || context.nodes >= SNAPSHOT_MAX_NODES) {
    throw new TypeError('untrusted-data-shape-invalid');
  }
  context.nodes += 1;
  if (value == null || (typeof value !== 'object' && typeof value !== 'function')) {
    if (typeof value === 'symbol' || typeof value === 'function') throw new TypeError('untrusted-data-type-invalid');
    return value;
  }
  if (typeof value === 'function' || context.seen.has(value)) {
    throw new TypeError('untrusted-data-shape-invalid');
  }
  ...
  try {
    ...
    if (isArray ? prototype !== Array.prototype : prototype !== Object.prototype && prototype !== null) {
      throw new TypeError('untrusted-data-prototype-invalid');
    }
    const keys = Reflect.ownKeys(value);
    const remainingNodes = SNAPSHOT_MAX_NODES - context.nodes;
    if (keys.length > remainingNodes + (isArray ? 1 : 0)) throw new TypeError('untrusted-data-node-limit');
    if (keys.some((key) => typeof key !== 'string')) throw new TypeError('untrusted-data-key-invalid');
    if (isArray) {
      const lengthDescriptor = Reflect.getOwnPropertyDescriptor(value, 'length');
      if (!lengthDescriptor || !Object.hasOwn(lengthDescriptor, 'value')
        || !Number.isSafeInteger(lengthDescriptor.value) || lengthDescriptor.value < 0) {
        throw new TypeError('untrusted-array-length-invalid');
      }
      const length = lengthDescriptor.value;
      if (length > remainingNodes || keys.length !== length + 1 || keys[keys.length - 1] !== 'length') {
        throw new TypeError('untrusted-array-keys-invalid');
      }
      for (let index = 0; index < length; index += 1) {
        if (keys[index] !== String(index)) throw new TypeError('untrusted-array-keys-invalid');
      }
      const result = [];
      for (let index = 0; index < length; index += 1) {
        ...
        if (!descriptor || !Object.hasOwn(descriptor, 'value') || descriptor.enumerable !== true) {
          throw new TypeError('untrusted-data-accessor-invalid');
        }
        result[index] = snapshotUntrustedData(descriptor.value, context, depth + 1);
      }
      return result;
    }
    ...
    for (const key of keys) {
      ...
      result[key] = snapshotUntrustedData(descriptor.value, context, depth + 1);
    }
    return result;
  } finally {
...
async function runBounded(thunk, { signal = null, deadline }) {
  if (checkCancelled(signal)) return { status: 'cancelled' };
  const remaining = deadline - Date.now();
  if (!Number.isFinite(remaining) || remaining <= 0) return { status: 'resource-limited' };
  ...
  return new Promise((resolve) => {
    ...
    const timer = setTimeout(() => {
      controller.abort('machine-effects-oracle-time-budget-exceeded');
      finish({ status: 'resource-limited' });
    }, remaining);
...
function safeResult({
  ...
  const safeIdentity = (() => {
    try { return assertDistinctOracleIdentity(oracleIdentity ?? caseValue.oracleIdentity, 'oracle-result-identity'); }
    catch { return caseValue.oracleIdentity; }
  })();
  const safeVersion = (() => {
    const candidate = String(oracleVersion ?? caseValue.oracleVersion);
    return candidate.trim() === '' ? caseValue.oracleVersion : candidate;
  })();
  const safeSource = (() => {
    try { return assertIndependentText(oracleSource ?? caseValue.expectedStateSource.executionSource, 'oracle-result-source'); }
    catch { return caseValue.expectedStateSource.executionSource; }
  })();
  const safeToolchain = (() => {
    try { return assertIndependentText(toolchainIdentity ?? caseValue.provenance.toolchainIdentity, 'oracle-result-toolchain'); }
    catch { return caseValue.provenance.toolchainIdentity; }
  })();
  const safeProvenance = (() => {
    try { return assertIndependentProvenance(provenance ?? caseValue.provenance, {
      oracleIdentity: safeIdentity,
      expected: caseValue.provenance,
      code: 'oracle-result-provenance',
    }); }
  ...
  try {
    ...
    return createOracleResult({
      ...
      oracleSource: caseValue.expectedStateSource.executionSource,
      ...
      comparisonCounts: { registers: 0, flags: 0, vectors: 0, memory: 0, outcome: 0, total: 0 },
      mismatches: [],
      diagnostics: [diagnostic('result-normalization-failed', safeThrownMessage(error))],
      passContribution: 0,
    }, caseValue);
  }
}
...
function oracleEnvelopeValid(oracle, caseValue) {
  ...
  try {
    ...
    assertIndependentProvenance(oracle.provenance, {
      oracleIdentity: oracle.identity,
      expected: caseValue.provenance,
      code: 'oracle-adapter-provenance',
    });
  } catch (error) {
    return diagnostic('oracle-adapter-provenance-invalid', safeThrownMessage(error));
  }
  ...
  if (oracle.toolchainIdentity !== caseValue.provenance.toolchainIdentity) return diagnostic('oracle-adapter-toolchain-mismatch');
  return null;
}
...
export async function runIndependentComparison({
  ...
  const deadline = startedAt + effectiveBudgets.timeoutMs;
  ...
  const adapter = oracle ?? createReferenceOracle({
    ...
    source: caseValue.expectedStateSource.executionSource,
    toolchainIdentity: caseValue.provenance.toolchainIdentity,
    provenance: caseValue.provenance,
  });
  ...
  const oracleExecution = await runBounded(
    (operationSignal) => adapter.evaluate(caseValue, { signal: operationSignal, budgets: effectiveBudgets }),
    { signal, deadline },
  );
  if (oracleExecution.status === 'cancelled') return safeResult({ caseValue, status: 'cancelled', diagnostics: [diagnostic('oracle-cancelled')] });
  if (oracleExecution.status === 'resource-limited') return safeResult({ caseValue, status: 'resource-limited', diagnostics: [diagnostic('time-budget-exceeded')] });
  if (oracleExecution.status === 'error') return safeResult({ caseValue, status: 'unavailable', diagnostics: [diagnostic('oracle-evaluation-error', safeThrownMessage(oracleExecution.error))] });
  const oracleObservation = oracleExecution.value;
  ...
  try { oracleObservationBytes = Buffer.byteLength(canonicalStringify(oracleObservation), 'utf8'); }
    ...
    return safeResult({
      ...
      toolchainIdentity: adapter.toolchainIdentity,
      provenance: adapter.provenance,
      diagnostics: [diagnostic('oracle-output-malformed', safeThrownMessage(error))],
    });
  }
  ...
  try { oracleOutcome = normalizeOutcome(oracleObservation.outcome, 'oracle-observed-outcome'); }
    ...
    return safeResult({
      ...
      toolchainIdentity: adapter.toolchainIdentity,
      provenance: adapter.provenance,
      diagnostics: [diagnostic('oracle-outcome-malformed', safeThrownMessage(error))],
    });
  }
  let oracleState;
  try { oracleState = normalizeMachineState(oracleObservation.state, 'oracle-observed-state'); }
    ...
    return safeResult({
      ...
      toolchainIdentity: adapter.toolchainIdentity,
      provenance: adapter.provenance,
      diagnostics: [diagnostic('oracle-state-malformed', safeThrownMessage(error))],
    });
  }
  ...
  const subjectExecution = await runBounded(
    (operationSignal) => subject({ caseValue, signal: operationSignal, budgets: effectiveBudgets }),
    { signal, deadline },
  );
  if (subjectExecution.status === 'cancelled') return safeResult({
    caseValue,
    status: 'cancelled',
    oracleIdentity: adapter.identity,
    oracleVersion: adapter.version,
    ...
    toolchainIdentity: adapter.toolchainIdentity,
    provenance: adapter.provenance,
    diagnostics: [diagnostic('production-subject-cancelled')],
  });
  if (subjectExecution.status === 'resource-limited') return safeResult({
    caseValue,
    status: 'resource-limited',
    oracleIdentity: adapter.identity,
    oracleVersion: adapter.version,
  ...
  if (subjectExecution.status === 'error') {
    const error = subjectExecution.error;
    return safeResult({
      caseValue,
      status: 'malformed',
      oracleIdentity: adapter.identity,
      ...
      toolchainIdentity: adapter.toolchainIdentity,
      provenance: adapter.provenance,
      diagnostics: [diagnostic('production-subject-error', safeThrownMessage(error))],
    });
  }
  const untrustedSubjectObservation = subjectExecution.value;
  if (!untrustedSubjectObservation || typeof untrustedSubjectObservation !== 'object') return safeResult({
    caseValue,
  ...
  try {
    subjectObservation = snapshotUntrustedData(untrustedSubjectObservation);
  } catch {
    return safeResult({
      caseValue,
      status: 'malformed',
      ...
      toolchainIdentity: adapter.toolchainIdentity,
      provenance: adapter.provenance,
      diagnostics: [diagnostic('production-subject-envelope-invalid')],
    });
  }
  const subjectKeys = Object.keys(subjectObservation).sort();
  if (canonicalStringify(subjectKeys) !== canonicalStringify(['outcome', 'state', 'subjectIdentity', 'subjectRole'])) return safeResult({
    caseValue,
    status: 'malformed',
    oracleIdentity: adapter.identity,
    oracleVersion: adapter.version,
  ...
  try {
    ...
    return safeResult({
      ...
      toolchainIdentity: adapter.toolchainIdentity,
      provenance: adapter.provenance,
      diagnostics: [diagnostic('production-subject-output-malformed', safeThrownMessage(error))],
    });
  }
  let observedOutcome;
  try { observedOutcome = normalizeOutcome(subjectObservation.outcome, 'production-subject-outcome'); }
    ...
    return safeResult({
      ...
      toolchainIdentity: adapter.toolchainIdentity,
      provenance: adapter.provenance,
      diagnostics: [diagnostic('production-subject-outcome-malformed', safeThrownMessage(error))],
    });
  }
  ...
  try {
    ...
    return safeResult({
      ...
      observedOutcome,
      observedState: null,
      diagnostics: [diagnostic('production-subject-state-malformed', safeThrownMessage(error))],
    });
  }
...
export async function runCorpus(corpus, options = {}) {
  ...
  while (results.length < normalizedCorpus.cases.length) {
    results.push(safeResult({
      caseValue: normalizedCorpus.cases[results.length],
      status: 'cancelled',
      diagnostics: [diagnostic('cancelled-before-case-run')],
    }));
  }
  ...
  const allUnsupported = results.length > 0 && results.every((result) => result.status === 'unsupported');
  return Object.freeze({
    corpusId: normalizedCorpus.corpusId,
    results: Object.freeze(results),
    counts: Object.freeze({ total: normalizedCorpus.cases.length, pass, blocking, gaps }),
    status: results.some((result) => result.status === 'cancelled') ? 'cancelled'
      : blocking > 0 ? 'mismatch'
        : gaps === 0 ? 'exact/equivalent'
