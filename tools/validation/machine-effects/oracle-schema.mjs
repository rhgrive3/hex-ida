...
function normalizeProvenance(value, oracleIdentity, expected, code = 'provenance-invalid') {
  plainObject(value, code);
  ...
  const normalized = assertIndependentProvenance(value, { oracleIdentity, expected, code });
  return normalized;
}
...
function normalizeCaseInput(input) {
  ...
  if (expectedStateSource.revision !== profile.revision) fail('corpus-case-profile-revision-mismatch');
  if (expectedStateSource.executionSource !== profile.executionSource) fail('corpus-case-profile-execution-source-mismatch');
  ...
  const provenance = normalizeProvenance(input.provenance, oracleIdentity, {
    authorityId: profile.authorityId,
    authorityRole: profile.authorityRole,
    isaReference: expectedStateSource.reference,
...
function normalizeResultInput(input, caseValue = null) {
  ...
  const provenance = normalizeProvenance(input.provenance, oracleIdentity, caseValue?.provenance ?? null, 'oracle-result-provenance');
