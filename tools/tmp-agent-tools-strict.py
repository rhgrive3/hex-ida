...
const malformedOffset=pageRows({ offset:['10'], results:rows }, 1, 11);
assert.deepEqual(malformedOffset.results, ['r10'], 'malformed upstream offset must not define page origin');
const goodOffset=pageRows({ offset:10, results:rows }, 1, 11);
assert.deepEqual(goodOffset.results, ['r11']);
assert.notEqual(pageRows({ results:[], coverage:['1'], complete:false }, 10, 0).coverage, 1, 'array coverage must not become full coverage');
assert.notEqual(pageRows({ results:[], coverage:'1', complete:false }, 10, 0).coverage, 1, 'numeric-string coverage must not become full coverage');
