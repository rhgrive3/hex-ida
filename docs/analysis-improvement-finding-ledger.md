...
| HEX-C4-03 | IN_PROGRESS | PARTIAL | Sol | `specs/004-render-provenance-mapping` | T001–T025 | `feat/analysis-hex-c4-03-provenance` | C4-01 (proven gate), C2-01 (published seams only) | rendered entities from many-to-one Phase 8 transforms carry forward origins but reverse navigation/completeness gating is unproven | in progress (spec/plan/tasks committed at `a8acd39f`) | pending | pending | — | — | deleted/merged entities may lose navigation | none (parallel lanes own different files) |
