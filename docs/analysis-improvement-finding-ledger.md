...
| HEX-X-01 | ACCEPTED-LOCAL | LOCAL_COMPLETE | existing/main | historical | X-01 complete | `feat/analysis-roadmap-v8-current-main-20260907` / PR 7036 | C0-01 | F6 rebuild transaction v2 (12 units, 4 profiles, 4 bounded cells), independent writer/oracle identity separation, canonical provider registration, fail-closed gate on missing/rejecting oracle, publication receipt boundary (isValidatedAtomicPublicationReceipt), format-safe ELF/PE/Mach-O rebuild, host llvm-readobj layout evidence, and pinned LLVM 18.1.3 environment exclusion verified | `tests/stage2/x01-format-safe-rebuild-acceptance.test.mjs`, `js/rebuild/**`, `tools/validation/rebuild-independent-oracle.mjs` | x01-format-safe-rebuild-acceptance 7/7 PASS | PASS | exact head green | — | non-regression; whole-roadmap/device gates remain open | none |
| HEX-X-02 | PARTIAL-LOCAL-MATRIX | PARTIAL | Sol | `docs/analysis-x02-acceptance.md` | X-02 partial | `feat/analysis-roadmap-v8-current-main-20260907` / PR 7036 | C3-03, X-01, X-03 | 120-row Apple matrix canonical integration; current overlay resolves X02-F-48 with compiler-produced arm64e PAC Mach-O; current finite classification 115 pass / 0 product-gap / 3 evidence-gap / 2 environment-excluded (116/0/3/1 with pinned LLVM 18.1.3); remaining evidence-gaps (Apple OS corpus, dyld cache provenance, trusted signing) and environment exclusions (physical PAC runtime, pinned LLVM oracle) strictly fail closed | `tests/scpa/x02-prior120-apple-version-matrix.test.mjs`, `tests/scpa/x02-apple-version-matrix.test.mjs`, `tests/scpa/x02-declared-metadata-boundaries.test.mjs`, `docs/analysis-x02-acceptance.md` | prior120 120/120 PASS, apple-version-matrix 61/61 PASS, declared-metadata-boundaries 12/12 PASS | PASS | exact head green | — | real Apple runtime/cache/signing and PAC hardware execution remain open; synthetic markers never promoted to proof | none |
| HEX-X-03 | ACCEPTED-LOCAL | LOCAL_COMPLETE | Sol | `specs/001-rebuild-v2-impact-discovery` | X-03 complete | `feat/analysis-roadmap-v8-current-main-20260907` / PR 7036 | C0-01, ME-01, X-01 | ambiguity-preserving discovery artifact (hex-discovery-ambiguity-artifact/v1), code/data & function overlap retention, reference preservation (jump-table & relocation symbolic expressions), anti-promotion (rank/confidence neutrality), rebuild transaction binding (isFactoryIssuedDiscoveryRebuildBinding), reparse gate anti-loss & anti-silent-promotion, and format-safe production adapter integration verified | `tests/stage2/x03-rebuild-discovery-acceptance.test.mjs`, `tests/stage2/x03-rebuild-discovery.test.mjs`, `tests/phase7/discovery/x03-ambiguity-artifact.test.mjs` | x03-rebuild-discovery-acceptance 7/7 PASS, x03-rebuild-discovery PASS, x03-ambiguity-artifact 13/13 PASS | PASS | exact head green | — | ranked candidate must not become exact truth; full format/architecture corpus breadth separate | none |
