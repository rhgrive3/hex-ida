...
| HEX-C4-03 | VERIFYING | COMPLETE_PENDING_MERGE | Sol | `specs/005-phase8-provenance-complete` | T001–T007 complete 2026-09-01 | `feat/analysis-partial-closure-20260901` | C4-01, C2-01 | bidirectional provenance completed on campaign head: transform records union consumed origin ids onto produced sources; frozen deterministic per-line `lineProvenance` published (version 2); `verifyLineProvenance` fail-closed verifier rejects forged ids | `js/decompiler/phase8/projection.js` + `tests/phase8/projection/line-provenance.test.mjs` | focused 5/5; projection/integration 13/13; `npm run phase8:test` PASS (1714s quiet) with pseudocode text unchanged | spec/plan/tasks complete; converge n/a | local exact-head verifier READY; GitHub CI pending push/PR | — | rendering ids remain projections, not semantic identities | none |
