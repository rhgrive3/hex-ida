...
| HEX-ME-01 | REMAINING | PARTIAL | Sol | `specs/003-oracle-mask-matrix` | T001 complete (infrastructure mapped); T003/T005 phase-1 denominator locked; T002/T004/T006 pending | `feat/analysis-hex-me-01-oracle-matrix` @ `3ccb0b1b` (unmerged, phase 1) | C0-01 | 9-case phase-1 matrix on `3ccb0b1b`: orderings (ldar/stlr/ldxr) preserve or downgrade-to-unknown, never upgrade; non-atomic+ordering and bogus ordering rejected at contract; variable-shift modulo, sdiv zero/overflow named, unmodelled stays partial | `tests/machine-effects/ordering-undefined-matrix.test.mjs` frozen denominator; oracle infrastructure unchanged (`EXTERNAL_ORACLE_POLICY` reuse) | matrix 9/9; canonical machine-effects runner PASS (184.1s) on exact head `3ccb0b1b` | — | — | — | incomplete observables may create false confidence | none |
...
| HEX-C1-02 | REMAINING | PARTIAL | Sol | `specs/002-return-pointer-summaries` | T001–T004 complete (matrix locked); T005–T009 evidence recorded | `feat/analysis-hex-c1-02-phase2` @ `d37fe9c5` (unmerged, phase 2) | C1-03 | 13-case target matrix locked on `d37fe9c5`: every incompleteness axis (missing summary, targetless call, pinned-identity mismatch, partial status, unknown call effects, empty/misindexed provenance, top argument, absent argument, malformed offset, unknown provenance kind, recursion) stays `unresolved-call`; complete callee joins precisely | 12-axis return-summary consumer matrix in `tests/phase7/summary/c1-02-target-matrix.test.mjs`; no production change needed — current main already conservative on all axes | matrix 13/13; canonical Phase 7 runner PASS (17.0s) on exact head `d37fe9c5`; lint PASS | pending PR + Sol review | — | — | PR 2434 floor intact (loaded-pointer regression PASS); production wiring of pinned `summaryAnalyzerId` remains optional and unpinned by default | none |
...
| HEX-X-03 | REMAINING | PARTIAL | Sol | `specs/004-discovery-ambiguity-matrix` | T001–T003 complete (matrix locked); T004 n/a (no production gap found); T005–T006 evidence recorded | `feat/analysis-hex-x03-discovery-matrix` @ `8ed2d53a` (unmerged, phase 1) | C0-01, ME-01, X-01 | 10-case phase-1 matrix on `8ed2d53a`: swallow/partial overlaps record conflicts and reset both extents, boundary-adjacency stays clean, shared ownership stays representable, vtable/relocation references never mint exact starts, authority ladder holds (heuristic/probable/exact), reparse in reverse order is digest-identical | `tests/phase7/discovery/ambiguity-matrix.test.mjs` frozen matrix over `fuseFunctionCandidates` + `referenceProducer`; no production change needed — current main already fail-closed on all cases | matrix 10/10; Phase 7 runner PASS (20.4s); lint PASS on exact head `8ed2d53a` | — | — | — | ranked candidate must not become exact truth | none |
