...
| HEX-ME-01 | REMAINING | PARTIAL | Sol | `specs/003-oracle-mask-matrix` | T001–T005 implemented; T006 transport gap check passed; T007 executed (FAIL); T008 open | `feat/analysis-roadmap-v8-current-main-20260907` / existing PR 7036 | C0-01 | Phase 2 pinned Isla/herd/Sail/QEMU artifacts retained; Phase 1 now freezes six ordering and four undefined-mask transport cases with separate claim-local litmus references | real V2/SSA/MemorySSA/V1 transport observations; downstream loss/precision mutations and unmodified-copy controls; no new semantic engine or architectural authority | source `4c855b38c` focused 33/33, lint and inventory PASS; canonical 13/220 files FAIL (one excluded browser subtree stopped), full green unproven | independent read-only source/test/docs review recorded; Sol-specific review unclaimed | — | — | physical-hardware triangulation, arm64e PAC, modern x86 formal evidence, RV64 relaxed-memory and architecture-wide undefined-state coverage remain absent; full suite is not green | SYM-01/X-03 and issue/environment work remain user-owned |
