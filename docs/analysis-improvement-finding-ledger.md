...
| HEX-X-02 | VERIFYING | PARTIAL | Sol | `specs/005-apple-versioned-knowledge` | T001–T008, T010–T022 complete; T009 external oracle blocked | `work/x02-complete` | C3-03, X-01, X-03 | hardened counterexample matrix covers immutable lexical Swift/ObjC probes, prototype/instance/subclass/proxy-result rejection, parser-owned sections/readers and complete absence, copied/missing/custom reader rejection, byte-derived/private artifact authority, clone/manual rejection, malformed/truncated signing discovery, exact chained page coverage and early-budget ownership, cache-bound format 4, CodeDirectory infinite-page/hash/reserved/version fields, v2.3 `codeLimit64`, exact source-backed/fat provenance, duplicate sites, storage width, PAC fields, and serialized-authority rejection | canonical `macho-dyld` pointer truth plus module-issued immutable `apple/knowledge` matrix; serialized evidence is always unknown/incomplete; Mach-O retains bounded SuperBlob/CodeDirectory provenance without validity claims | X-02 focused, binary/source, metadata, chained and internal rebuild suites PASS; Phase 12 independent fixture remains blocked because `llvm-readobj` is unavailable | Spec/code/test convergence PASS | local exact-tree lint/module/ownership/diff gates green; independent oracle unavailable | — | required `llvm-readobj` writer-independent fixture proof and post-merge exact-main verification | none |
