...
| HEX-X-03 | VERIFYING | COMPLETE | Sol | `specs/004-discovery-ambiguity-matrix` | T001–T036 complete | `work/x03-complete` (local; no PR per task) | C0-01, ME-01, X-01 | pre-fix fusion withdrew disputed extents and left rebuild/ToolRegistry without the alternative interval set | immutable typed discovery artifact retains unresolved alternatives; partial extents require explicit coverage, interval IDs are canonical/unique, candidates and ToolRegistry are descriptor-only, and relevant rebuilds require live output-byte-bound discovery attestation | X-03 20/20; focused discovery/AI/rebuild 59/59; Phase 7 PASS; Stage 2 rebuild PASS; syntax 1,809 PASS; module/ownership/diff PASS | PASS | local exact-head X-03 verifier READY; full check remains BLOCKED outside scope by unavailable LLVM MC 18 plus untouched MachineEffects assertion; optional unlinked batch 1 also fails in untouched Semantic IR | — | Apple-specific chained-fixup/PAC/signing promotion remains X-02; no caller IDs, getters, copied hashes, kind/authority string, or candidate score can mint exact truth | none |
