...
The legacy prior-120 test keeps the frozen row input and historical disposition intact. `tests/scpa/x02-current-acceptance-overlay.test.mjs` is the current disposition overlay that promotes only the already-remediated six product gaps plus `X02-F-48`; it does not mutate the frozen matrix.
