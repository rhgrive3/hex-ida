...
| 2 | Rescue retained at regression = 0 | **Full corpus: 0 positive-rescue gates** in the pre-registered catalog achieve regression=0 (only net-0 gates: G7/G18/G22). Primary development maxReg=0 selection (G9) **leaks 1 regression on holdout** (rescue 6 / reg 1). Holdout diagnostic: G28_strong_only achieves **14 rescue / 0 regression** on TsumTsum+YWP but was selected under maxReg≤1, not maxReg=0. |
| 3 | Rescue retained at regression ≤ 1 | **Best validated gate G28_strong_only**: development (BattleCats) rescue **15** / reg **1**; **holdout (TsumTsum+YWP) rescue 14 / reg 0**; full corpus rescue **29** / reg **1**. |
| 4 | Best holdout-tested gate | **`G28_strong_only`** — fire label-only Jev preference only when baseline P4 verdict is strong (`confirmed` or `likely`) on partial queries. Selected on BattleCats with regression ≤ 1; confirmed on TsumTsum+YWP. |
| 5 | top1 on current main after that gate | **310/426 (72.8%)** vs baseline **282/426 (66.2%)** (+28). |
...
| 9 | Production? | **CONDITIONAL_GO** only for `G28_strong_only`-style **strong-verdict-gated label preference** (preference-only, fail-closed baseline fallback, never mint facts, never change verdicts). Force-all and ambiguous-only remain **NO_GO** for production (7 and 6 regressions). Broader partial Jev stays **RESEARCH_ONLY** until an independent free-form intent holdout exists. This PR does not integrate production. |

## Comparison table (required)

| Row | correct top1 (overall / partial) | wrong→correct | correct→wrong | net | triggered | API calls | false-strong Δ |
| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
...
| Best zero-regression gate — holdout (Tsum+YWP) | — | 6 | 1 | 5 | 42 | 42 | 0 |
| Best ≤1 regression gate (`G28_strong_only`) — holdout | — | 14 | 0 | 14 | 52 | 52 | 0 |
| Best validated gate (holdout-selected G28) — full corpus | 310 / 81 | 29 | 1 | 28 | 91 | 91 | 0 |
...
Verdict policy is never rewritten. **false-strong Δ = 0** means preference-only top1 movement without `confirmed`/`likely` promotion. Overwriting a *correct strong* top1 is still counted in `strong-override-arms.json` → `newlyBrokenStrong` (G28 full-corpus breaks 1).
...
1. Gate catalog is **pre-registered** in `analyze.mjs` (G0–G34).
2. **Primary split:** development = BattleCats (86 partial), holdout = TsumTsum+YWP (110 partial).
3. Selection objective on development only: maximize net rescue subject to regression ≤ 0 or ≤ 1.
4. **Leave-one-binary-out** folds for robustness.
5. No post-hoc threshold scrubbing on holdout.

| Selection | Gate | Dev (rescue/reg) | Holdout (rescue/reg) | Full (rescue/reg) |
| --- | --- | ---: | ---: | ---: |
...
**LOO:** with maxReg ≤ 1, every fold selects `G28_strong_only`; test folds give (Tsum 8/0), (YWP 6/0), (battlecats 15/1). With maxReg = 0, folds are unstable (G9/G14/G22) and do not hold at 0.
...
| Catalog-bounded observable | best pre-registered gate (truth used only to score) | zero-reg: net 0; ≤1-reg: 29/1 |
...
**CONDITIONAL_GO** for predicate **`G28_strong_only`** only:

- Holdout net gain **+14 top1 on partial** with **0 regressions** (TsumTsum+YWP).
- Full corpus **+28 top1** (282→310), partial **53→81**, one accepted regression.
- Deterministic comparator still dominates raw accuracy; Jev-specific fill ≈ 5–6 rows.
- Repeated-call evidence **argues against** stability-based gating (regressions are stable).
- Fixture/SDK sharing across binaries is **not** an independent intent holdout → production merge still requires a new free-form query holdout.

**NO_GO** for force-all (7 regressions) and ambiguous-only-as-default (6 regressions).

**RESEARCH_ONLY** for any broader partial/ambiguous Jev surface and for semantic naming of unnamed fields.
...
Raw `current-main-baseline-rows.jsonl` (3.7 MB) and live checkpoints stay in local evidence storage; the committed projection is `row-classification.jsonl` + case files + `current-main-baseline.json`.

## Prohibitions observed

- No production ranking / verdict / Jev integration changes in this PR.
- Truth never enters gate predicates (machine-checked).
- No same-dataset gate claimed as validated without a holdout split.
- No favorable-only split reporting; LOO folds included.
