...
const SOURCE = path.resolve(arg('--input', path.join(ROOT, 'reports/investigations/jev-rescue-regression-gap/current-main-baseline-rows.jsonl')));
const CHECKPOINT = path.resolve(arg('--checkpoint', path.join(HERE, 'checkpoint')));
const SEED = arg('--seed', '');
const MODE = arg('--mode', 'live');
const ARMS = (arg('--arms', 'label') || '').split(',').filter(Boolean);
...
function projectLive(x, r) {
  ...
  return {
    ...
    preference: x.preference ?? null, fallback: x.fallback ?? 0,
  };
}
...
function seedFile(rowId, arm) {
  if (!SEED) return null;
  const file = path.join(SEED, `${sha(`${rowId}|${arm}`)}.json`);
  if (!fs.existsSync(file)) return null;
  const saved = JSON.parse(fs.readFileSync(file, 'utf8'));
  if (saved.rowId !== rowId || saved.arm !== arm) return null;
  return saved;
}
...
async function liveOne(r, arm) {
  ...
  if (fs.existsSync(file)) {
    const saved = JSON.parse(fs.readFileSync(file, 'utf8'));
    if (saved.bodyHash === bodyHash && saved.rowId === rowId && saved.arm === arm && !saved.error) return saved;
    if (saved.bodyHash === bodyHash && saved.rowId === rowId && saved.arm === arm && saved.error && saved.error !== 'missing-key') return saved;
  }
  const seeded = seedFile(rowId, arm);
  if (seeded && seeded.bodyHash === bodyHash && !seeded.error) {
    safeWrite(file, seeded);
    return seeded;
  }
