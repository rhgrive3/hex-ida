...
- **Which method (first match wins):**
  ...
  4. **All three unavailable (opencode, Cline, Freebuff)** → `codex -m gpt-6-luna -c model_reasoning_effort=max`. Last route only; go back as soon as one of the others works.
- **Cline (`./cline-<N>`):** numbers `1`–`8` first; `9` may share an account with another number, so use it only when 1–8 are not enough, their free quota is gone, or the user allows it. Models in order, each at the highest reasoning the model offers (`xhigh` is the CLI maximum): `DeepSeek v4.1 Flash` (`cline-free/deepseek-v4.1-flash`) → `Muse Spark 1.3 Contributor` (`cline-free/muse-spark-1.3-contributor`) → `MiMo 2.6 Flash` (`cline-free/mimo-v2.6-flash`) → `Gemini 3.8 Flash` (`cline-free/gemini-3.8-flash`) → `Space Bunny Alpha` (`stealth/space-bunny-alpha`). One model's daily quota running out is not a reason to stop: switch model or number. Cline can stop silently; if it may have stopped, check its artifacts, `git diff`, and logs, then hand off.
- **Freebuff (`./freebuff-<N>`):** numbers `1`–`9`. `DeepSeek v4.1 Flash` high while the wallet has room; when about 10 remains (savings included) use `MiMo 2.6 Flash` — never spend the last ~10 on DeepSeek. `MiMo 2.6 Flash` is always selectable, so a Freebuff number is never out of models. The wallet, savings included, may be spent.
