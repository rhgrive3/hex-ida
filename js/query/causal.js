...
export function functionPaths(program, from, to, opts) {
  ...
  while (q.length && result.paths.length < maxPaths) {
    ...
    let range = null;
    try { range = program.functionRange(head); } catch { reasons.add('function-range-error'); range = null; }
    let callees = [];
    try { callees = program.calleesOf(head, range && range.end, 201) || []; } catch { reasons.add('callee-query-error'); callees = []; }
