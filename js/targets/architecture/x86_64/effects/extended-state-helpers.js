...
export function vectorIndex(operand){const match=/^(?:xmm|ymm|zmm)(\d+)$/.exec(registerName(operand));return match?Number(match[1]):null;}
export function isVectorOperand(operand){return operand?.type==='register'&&vectorIndex(operand)!=null;}
export function isMaskOperand(operand){return operand?.type==='register'&&/^k[0-7]$/.test(registerName(operand));}
export function vectorPrefixOffset(instruction,prefix){
  const raw=Uint8Array.from(instruction?.rawBytes||[]),reported=Number(instruction?.detail?.prefixes?.vector?.offset);
  ...
  const legacy=new Set([0xf0,0xf2,0xf3,0x2e,0x36,0x3e,0x26,0x64,0x65,0x66,0x67]);let cursor=0;
  while(cursor<raw.length&&legacy.has(raw[cursor]))cursor+=1;
...
export function evexInfo(instruction){
  const vector=instruction?.detail?.prefixes?.vector,bytes=Uint8Array.from(vector?.bytes||[]);
...
export function vexInfo(instruction){
  const vector=instruction?.detail?.prefixes?.vector,bytes=Uint8Array.from(vector?.bytes||[]),kind=String(vector?.kind||'').toLowerCase();
  const offset=vectorPrefixOffset(instruction,bytes);if(offset==null)return null;
  if(kind==='vex2'&&bytes.length===2&&bytes[0]===0xc5)return{kind,width:(bytes[1]&4)?256:128,vvvv:(bytes[1]>>>3)&15,prefixLength:2,prefixOffset:offset,map:1,pp:bytes[1]&3};
  if(kind==='vex3'&&bytes.length===3&&bytes[0]===0xc4){const map=bytes[1]&31;if(map<1||map>3)return null;return{kind,width:(bytes[2]&4)?256:128,vvvv:(bytes[2]>>>3)&15,prefixLength:3,prefixOffset:offset,map,pp:bytes[2]&3};}
  return null;
}
