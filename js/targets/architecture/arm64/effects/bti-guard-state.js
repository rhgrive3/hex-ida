...
/* Strict-identity dedupe for intrinsic inputs. A re-decorated bundle already
 * carries the guard-state inputs, so the rebuild must not list one input value
 * twice; order is preserved, which keeps metadata.inputOrder positional. */
function uniqueInputs(values) {
  const out = [];
  for (const value of values) if (value != null && !out.includes(value)) out.push(value);
  return out;
}
