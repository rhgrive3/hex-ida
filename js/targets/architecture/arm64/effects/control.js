import {
  ...
  directTargetOf,
  canonicalArm64InstructionAddress,
  immediateOf,
  instructionBits,
} from './common.js';
...
function instructionAddress(instruction) {
  return canonicalArm64InstructionAddress(instruction?.address);
}
...
function legacySymbolicDirectCallTarget(instruction, operand) {
  ...
  // Only the assembly-text semantic model may retain an unresolved symbol.
  // Decoder-origin records require their imm26/address evidence instead.
...
function liftArm64ControlEffectsCore(instruction, options = {}) {
  ...
  if (mnemonic === 'bl') {
    ...
    const controlTarget = target != null ? addressRef(target) : { kind:'symbolic-code-reference', name:symbolicTarget };
    const address = instructionAddress(instruction);
    if (address == null) {
