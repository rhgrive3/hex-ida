...
export function parseDynamicSymbolVersions(r,tags,image,symbolCount,stringAt,context=null){
  ...
  if(verdefPair.valid&&verdef!=null&&verdefnum){const range=mappedELFFileRangeForVa(image,verdef);let p=range?.start??null,decoded=0;for(let i=0;p!=null&&i<Math.min(verdefnum,65536)&&!budget.stopped;i++){
    if(!budget.step(1,'DT_VERDEF decode'))break;if(p+20>range.end){partial(image,'DT_VERDEF crosses a file-backed PT_LOAD boundary');break;}if(!budget.claimInput(20,'DT_VERDEF'))break;decoded++;const version=r.u16(p);if(version!==1){partial(image,`DT_VERDEF structure version ${version} is not supported (expected 1)`);break;}const ndx=r.u16(p+4)&0x7fff,cnt=r.u16(p+6),aux=r.u32(p+12),next=r.u32(p+16),ap=p+aux;
  ...
  if(verneedPair.valid&&verneed!=null&&verneednum&&!budget.stopped){const range=mappedELFFileRangeForVa(image,verneed);let p=range?.start??null,decoded=0;for(let i=0;p!=null&&i<Math.min(verneednum,65536)&&!budget.stopped;i++){
    if(!budget.step(1,'DT_VERNEED decode'))break;if(p+16>range.end){partial(image,'DT_VERNEED crosses a file-backed PT_LOAD boundary');break;}if(!budget.claimInput(16,'DT_VERNEED'))break;decoded++;const version=r.u16(p);if(version!==1){partial(image,`DT_VERNEED structure version ${version} is not supported (expected 1)`);break;}const cnt=r.u16(p+2),file=stringAt(BigInt(r.u32(p+4))),aux=r.u32(p+8),next=r.u32(p+12);let ap=p+aux;
