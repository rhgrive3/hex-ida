...
export function parseDynamicSymbolVersions(r,tags,image,symbolCount,stringAt,context=null){
  ...
  if(verdefPair.valid&&verdef!=null&&verdefnum){const range=mappedELFFileRangeForVa(image,verdef);let p=range?.start??null,decoded=0;for(let i=0;p!=null&&i<Math.min(verdefnum,65536)&&!budget.stopped;i++){
    if(!budget.step(1,'DT_VERDEF decode'))break;if(p+20>range.end){partial(image,'DT_VERDEF crosses a file-backed PT_LOAD boundary');break;}if(!budget.claimInput(20,'DT_VERDEF'))break;
    // The Elf*_Verdef structure version must be 1 (ELF gABI): anything else is
    // a layout we do not decode, not a record to reinterpret (#6181).
    if(r.u16(p)!==1){partial(image,'DT_VERDEF record version is not 1');break;}
    decoded++;const ndx=r.u16(p+4)&0x7fff,cnt=r.u16(p+6),aux=r.u32(p+12),next=r.u32(p+16),ap=p+aux;
  ...
  if(verneedPair.valid&&verneed!=null&&verneednum&&!budget.stopped){const range=mappedELFFileRangeForVa(image,verneed);let p=range?.start??null,decoded=0;for(let i=0;p!=null&&i<Math.min(verneednum,65536)&&!budget.stopped;i++){
    if(!budget.step(1,'DT_VERNEED decode'))break;if(p+16>range.end){partial(image,'DT_VERNEED crosses a file-backed PT_LOAD boundary');break;}if(!budget.claimInput(16,'DT_VERNEED'))break;
    // Elf*_Verneed structure version must be 1 as well (#6181).
    if(r.u16(p)!==1){partial(image,'DT_VERNEED record version is not 1');break;}
    decoded++;const cnt=r.u16(p+2),file=stringAt(BigInt(r.u32(p+4))),aux=r.u32(p+8),next=r.u32(p+12);let ap=p+aux;
