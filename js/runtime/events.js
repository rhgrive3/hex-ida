...
function normalizeCompleteness(value, fallback = 'partial') {
  ...
  if (typeof completeness !== 'string' || !EVIDENCE_COMPLETENESS.includes(completeness)) {
    throw new DebugAdapterError('runtime-invalid-completeness', 'invalid runtime completeness');
  }
  return completeness;
}
...
function normalizeMode(value) {
  ...
  if (typeof mode !== 'string' || !RUNTIME_OBSERVATION_MODES.includes(mode)) {
    throw new DebugAdapterError('runtime-invalid-observation-mode', 'invalid runtime observation mode');
  }
  return mode;
}
...
export function createRuntimeEvent(input = {}) {
  ...
  const providerVersion = required(input.providerVersion ?? '1', 'runtime-provider-version-invalid', 'runtime providerVersion must be a non-empty string');
...
export function normalizeLegacyRuntimeEvent(input, context = {}) {
  ...
  const legacyCandidate = source.kind ?? source.type ?? 'trace-marker';
  const legacyType = protocolEnvelope ? input.event : (typeof legacyCandidate === 'string' ? legacyCandidate : 'trace-marker');
  const kindMap = {
    branch: 'basic-block',
    trace: 'trace-marker',
