const BUILTIN={v:'void',b:'bool',c:'char',a:'signed char',h:'unsigned char',s:'short',t:'unsigned short',i:'int',j:'unsigned int',l:'long',m:'unsigned long',x:'long long',y:'unsigned long long',n:'__int128',o:'unsigned __int128',f:'float',d:'double',e:'long double',z:'...',Ds:'char16_t',Di:'char32_t',Du:'char8_t',Dn:'decltype(nullptr)'};
...
export function demangleCxx(name){
  ...
  try{
    ...
    if(p.i<s.length){const args=readArgs(p);if(args==null||p.i!==s.length)return null;const m=/^(.*?)( (?:const|volatile|restrict)(?: (?:const|volatile|restrict))*)$/.exec(out);out=m?`${m[1]}(${args})${m[2]}`:`${out}(${args})`;}
...
function readNested(p){const parts=[];let cvr='';while(p.i<p.s.length&&/[rVK]/.test(p.s[p.i])){const q={r:' restrict',V:' volatile',K:' const'}[p.s[p.i++]];cvr=q+cvr;}while(p.i<p.s.length&&p.s[p.i]!=='E'){const c=p.s[p.i];let part=null;if(/\d/.test(c))part=readSourceName(p);else if(c==='S')part=readSubstitution(p);else if(c==='C'){if(p.i+2>p.s.length)return null;p.i+=2;part=parts.at(-1)||'ctor';}else if(c==='D'){if(p.i+2>p.s.length)return null;p.i+=2;part='~'+(parts.at(-1)||'dtor');}else if(OPERATORS[p.s.slice(p.i,p.i+2)]){part=OPERATORS[p.s.slice(p.i,p.i+2)];p.i+=2;}else if(c==='I')part=readTemplateArgs(p);else return null;if(!part)return null;parts.push(part);}if(p.s[p.i]!=='E'||!parts.length)return null;p.i++;return parts.reduce((a,b)=>b.startsWith('<')?a+b:(a?`${a}::${b}`:b),'')+cvr;}
...
export function demangleSwift(name){if(typeof name!=='string'||!name)return null;const s=name.startsWith('_$')?name.slice(1):name;if(!/^(?:\$s|\$S|_T0)/.test(s))return null;const body=s.replace(/^(?:\$s|\$S|_T0)/,'');const parts=[];let i=0,guard=0;while(i<body.length&&guard++<200){const m=/^(\d+)/.exec(body.slice(i));if(m){const len=Number(m[1]);i+=m[1].length;if(!Number.isSafeInteger(len)||len<=0)return null;const word=body.slice(i,i+len);if(word.length!==len)return null;parts.push(word);i+=len;continue;}const c=body[i];if(SWIFT_KIND[c])parts.push(`(${SWIFT_KIND[c]})`);i++;}if(!parts.length)return null;const words=parts.filter(x=>!x.startsWith('(')),kinds=parts.filter(x=>x.startsWith('(')),kind=kinds.length?kinds[0].replace(/[()]/g,''):null;if(!words.length)return null;return words.join('.')+(kind?`   [${kind}]`:'');}
