...
// Current-main metadata guardrails (#5496). These remain the canonical
// acceptance limits while the stricter query-hash validator below recomputes
// content identity after structured cloning.
...
export function createVerificationQuery({
  ...
  const normalizedTargetInput = normalizeTargetEntity(targetEntity);
  ...
  // Preserve current-main #5496 guardrails before any hashing walk. This keeps
  // adversarial deep/wide metadata on a deterministic domain-error path.
  freezeDeep(normalizedTargetInput);
  ...
  const normalizedTargetEntity = immutableIdentitySnapshot(normalizedTargetInput);
