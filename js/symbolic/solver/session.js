...
const identity = validateVerificationQuery(query, {
maxExprNodes: this.backend.maxExprNodes || 100000,
maxExprDepth: this.backend.maxExprDepth || 1024,
});
if (!identity.valid) {
return this._result(
identity.limitExceeded ? SOLVER_STATUS.RESOURCE_LIMIT : SOLVER_STATUS.INVALID_QUERY,
identity.reason,
{ budgetExceeded: identity.limitExceeded === true },
null,
);
}
trustedQueryHash = computeCanonicalQueryHash(query);
}
...
? validateVerificationQuery(query, {
maxExprNodes: this.backend.maxExprNodes || 100000,
maxExprDepth: this.backend.maxExprDepth || 1024,
})
: null;
let finalQueryHash = record.queryHash;
if (finalIdentity?.valid) {
try { finalQueryHash = computeCanonicalQueryHash(query); }
catch { finalQueryHash = null; }
}
if (finalIdentity && (!finalIdentity.valid || finalQueryHash !== record.queryHash)) {
result = this._result(
...
Promise.resolve()
.then(() => record.settled ? undefined : this._executeCheck(query, { ...options, timeoutMs, signal: controller.signal }, token, controller.signal))
