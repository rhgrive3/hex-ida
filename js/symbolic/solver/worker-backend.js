...
class WorkerSolverSession extends SolverSession {
  ...
  _attachWorker(worker) {
    const onMessage = (event) => {
      ...
      const identityOk = message.token === pending.token && isValidSolverResult(message.result, {
  ...
  async _executeCheck(query, options = {}, token, signal) {
    ...
    const validation = validateVerificationQuery(query, { maxExprNodes: workerOptions.maxExprNodes });
    if (!validation.valid) {
      return createSolverResult({
    ...
    if (!clonedValidation.valid || clonedValidation.recomputedHash !== validation.recomputedHash) {
      return createSolverResult({
        status: SOLVER_STATUS.INVALID_QUERY,
    ...
    return new Promise((resolve) => {
      this.pending.set(requestId, {
        ...
        query: querySnapshot,
        symbols: route.collected.symbols,
      });
      try {
