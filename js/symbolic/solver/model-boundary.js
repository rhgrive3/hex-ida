...
/**
 * Validate an exact witness before semantic evaluation/publication.
 *
 * Current main's canonical solver binding namespace is symbolId-only (#6087).
 * Display names are deliberately not aliases here because a display name may
 * equal another symbol's canonical symbolId.
 */
export function validateExactModelBindings(symbols, model) {
  ...
  if (values.size !== entries.length) return Object.freeze({ valid: false, reason: 'duplicate-exact-model-key' });
  ...
  for (const symbol of symbols) {
    if (!values.has(symbol.symbolId)) {
      return Object.freeze({ valid: false, reason: `missing-exact-model-binding:${symbol.symbolId}` });
    }
    ...
    if (symbol.sort.kind === SORT_KIND.BOOL) {
      ...
      return Object.freeze({ valid: false, reason: `unsupported-model-sort:${symbol.symbolId}` });
    }
  }
