...
export function validateExactModelBindings(symbols, model) {
  ...
  for (const symbol of symbols) {
    ...
    if (!hasName && !hasId) return Object.freeze({ valid: false, reason: `missing-exact-model-binding:${symbol.symbolId}` });
    const value = hasId ? values.get(symbol.symbolId) : values.get(symbol.name);
    if (hasName && hasId && !Object.is(values.get(symbol.name), values.get(symbol.symbolId))) {
