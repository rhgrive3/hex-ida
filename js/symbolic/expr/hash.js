  ...
  for (const root of roots) {
    ...
    while (stack.length > 0) {
      ...
      switch (node.kind) {
        ...
        default:
          if (!boundedAcyclicValue(node)) return Object.freeze({ ok: false, reason: 'malformed-generic-expression-node', nodeCount });
          try { canonicalRep = `GENERIC:${node.kind ?? ''}:${sortStr}:${stableDigest(node)}`; }
          catch { return Object.freeze({ ok: false, reason: 'malformed-generic-expression-node', nodeCount }); }
      }
