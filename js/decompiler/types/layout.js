...
export function recoverAggregateLayouts(ir, types, opts = {}) {
  ...
  for (const g of groups.values()) {
    ...
    if (kind !== 'array' && g.fixed.size >= 2) {
      ...
      // A group with no proven positive width has an unknown extent, not an
      // empty one: a zero-size extent would trivially never overlap and launder
      // the non-overlap proof for a field whose real width nobody observed.
      const noOverlap=fixed.every((f)=>f.size>0)
        && fixed.every((a,i)=>fixed.every((b,j)=>i===j || a.offset+BigInt(a.size)<=b.offset || b.offset+BigInt(b.size)<=a.offset));
