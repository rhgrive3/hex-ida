...
function reachingRegisterDefinition(ir, atInst, reg, control) {
  ...
  for (const instruction of instructions.value) {
    if (control?.isAborted?.()) return null;
    definitionCounts.set(instruction, (definitionCounts.get(instruction) || 0) + 1);
  }
  ...
  for (const value of values.value) {
    ...
    if (definitionCounts.get(def) !== 1) continue;
    if (defBlock === atBlock) {
  ...
  while (current != null && current >= 0 && guard-- > 0 && !seen.has(current)) {
    ...
    for (const instruction of instructions) {
      ...
      if (op === 'load') {
        ...
        if (!load || mutations.some((mutation) => memoryMutationCollides(load, mutation))) ambiguousRows.add(row);
        const loads = physicalLoads.get(row) || [];
        loads.push(load);
        physicalLoads.set(row, loads);
      } else if (['store', 'call', 'clobber', 'unknown'].includes(op)) {
        if (memoryRows.has(row)) ambiguousRows.add(row);
        const mutation = memoryMutationDescriptor(instruction);
        if (!mutation) { ambiguousRows.add(row); continue; }
        const loads = physicalLoads.get(row) || [];
        if (loads.some((load) => memoryMutationCollides(load, mutation))) ambiguousRows.add(row);
        memoryRows.add(row);
        const mutations = memoryMutations.get(row) || [];
    ...
    for (const inst of instructions) {
      ...
      if (op === 'store' && locationKind !== 'stack' && locationKind !== 'unknown'
        ...
        // Ambiguity before this defining store cannot change its later value.
        if ([...ambiguousRows].some(row => row >= fieldValue(inst, 'row'))) return null;
...
function semanticLocationForStore(result, store, control) {
  ...
  for (const item of result.semanticAst?.stores || []) {
    ...
    if (ids?.includes(storeId)) {
      const location = fieldValue(item, 'location');
      const physical = fieldValue(store, 'loc');
      if (fieldValue(location, 'kind') !== fieldValue(physical, 'kind')
          || fieldValue(location, 'key') !== fieldValue(physical, 'key')) return null;
...
function committedLocationForPhi(result, value, control) {
  ...
  const bits = fieldValue(value, 'bits');
  if (!trace || !viewTracePreservesPublishedWidth(trace, bits)) return null;
  // Canonical state reads may wrap a PHI in a width-preserving view chain.
  // Require each wrapper to be a unique live instruction before following it.
  const instructions = arrayField(result.ir, 'instructions');
  if (!instructions.ok) return null;
  ...
  for (let current = value; current !== trace.root;) {
    if (control?.isAborted?.()) return null;
    const def = fieldValue(current, 'def');
    if (counts.get(def) !== 1) return null;
    const args = arrayField(def, 'args');
    if (!args.ok || args.value.length !== 1) return null;
    current = valueOf(args.value[0]);
  }
  const definition = fieldValue(trace.root, 'def');
  if (counts.get(definition) !== 1) return null;
  ...
  const phiBlock = fieldValue(phi, 'block');
  if (!blocks.ok || !validBlock(phiBlock) || !blocks.value[phiBlock]) return null;
  const predecessors = arrayField(blocks.value[phiBlock], 'pred');
  const incomingBlocks = incomingField.value.map(item => fieldValue(item, 'from'));
  if (!predecessors.ok || predecessors.value.length !== incomingBlocks.length
      || new Set(incomingBlocks).size !== incomingBlocks.length
      || predecessors.value.some(block => !validBlock(block) || !incomingBlocks.includes(block))) return null;
  const locations = [];
...
/** Preserve a proven PHI value at its spill, before a later unknown call. */
export function recoverCommittedPhiSpillSnapshots(result, opts = {}) {
  if (!result?.semanticAst || !Array.isArray(result.cAst?.body)) return result;
  const control = recoveryControl(opts);
  const instructions = arrayField(result.ir, 'instructions');
  if (!instructions.ok) return result;
  const changes = [];
  for (const store of instructions.value) {
    if (control.isAborted()) return result;
    const loc = fieldValue(store, 'loc');
    if (fieldValue(store, 'op') !== 'store' || fieldValue(loc, 'kind') !== 'stack') continue;
    const size = positiveAccessSize(fieldValue(loc, 'size'));
    const args = arrayField(store, 'args');
    if (!size || !args.ok || args.value.length !== 1) continue;
    const value = valueOf(args.value[0]);
    const trace = exactViewTrace(value);
    if (fieldValue(fieldValue(trace?.root, 'def'), 'op') !== 'phi') continue;
    const phi = fieldValue(trace.root, 'def');
    const block = fieldValue(store, 'block');
    const row = fieldValue(store, 'row');
    if (!validBlock(block) || !validRow(row) || fieldValue(phi, 'block') !== block
        || !validRow(fieldValue(phi, 'row')) || fieldValue(phi, 'row') > row
        || fieldValue(value, 'bits') !== size * 8) continue;
    const location = committedLocationForPhi(result, value, control);
    if (!location) continue;
    let safe = true;
    for (const instruction of instructions.value) {
      if (control.isAborted()) return result;
      if (instruction === store || fieldValue(instruction, 'block') !== block) continue;
      const instructionRow = fieldValue(instruction, 'row');
      if (!validRow(instructionRow)) { safe = false; break; }
      if (instructionRow >= fieldValue(phi, 'row') && instructionRow <= row
          && committedStoreBarrier(instruction, location.key, control)) { safe = false; break; }
    }
    if (!safe) continue;
    const nodes = result.cAst.body.filter(node => node.semantic?.op === 'store'
      && node.semantic.ir === store.id && node.semantic.location?.key === loc.key);
    ...
    changes.push({ node, expression, stores:stores.map(item => ({ item, expression:item.expression })) });
  }
  if (!changes.length || control.isAborted()) return result;
  const publication = snapshotReturnPublication(result);
  const rollback = () => {
    for (const c of changes) for (const prior of c.stores) prior.item.expression = prior.expression;
    restoreReturnPublication(result, publication);
  };
  try {
    for (const c of changes) {
      c.node.semantic.expression = c.expression;
      for (const prior of c.stores) prior.item.expression = c.expression;
      c.node.text = `${c.node.semantic.location.text} = ${printExpression(c.expression)};`;
    }
