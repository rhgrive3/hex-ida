...
function memoryOwnData(object, key) {
...
function memoryFieldValue(object, key) {
  const field = memoryOwnData(object, key);
  return field.present && field.valid ? field.value : undefined;
}

/** Describe a physical memory operation without invoking accessor metadata. */
export function memoryMutationDescriptor(instruction) {
  const opField = memoryOwnData(instruction, 'op');
  const op = opField.present && opField.valid ? opField.value : null;
  if (op === 'load' || op === 'store') {
    const location = memoryFieldValue(instruction, 'loc');
    const kindField = memoryOwnData(location, 'kind');
    const keyField = memoryOwnData(location, 'key');
    const kind = kindField.present && kindField.valid ? kindField.value : null;
    const key = keyField.present && keyField.valid && typeof keyField.value === 'string'
      && keyField.value.length > 0 && keyField.value.trim() === keyField.value ? keyField.value : null;
    return { op, kind, key, broad:!PRECISE_MEMORY_LOCATION_KINDS.has(kind) || key == null };
  }
...
function numericOption(options, key, valid) {
  if (options === null || (typeof options !== 'object' && typeof options !== 'function')) return null;
  const descriptor = Object.getOwnPropertyDescriptor(options, key);
  if (!descriptor || !Object.hasOwn(descriptor, 'value')) return null;
  return valid(descriptor.value) ? descriptor.value : null;
}

function callbackOption(options, key) {
  return numericOption(options, key, (value) => typeof value === 'function');
}

function markRecoveryIncomplete(result, reason) {
  result.expressionHistoryBinding = Object.freeze({
    ...result.expressionHistoryBinding,
    completeness:'incomplete',
    reasons:Object.freeze([...new Set([...(result.expressionHistoryBinding?.reasons || []), reason])]),
  });
}
...
function storeValue(inst, key, values, expectedSize = null) {
  if (inst?.op !== 'store' || inst.loc?.kind !== 'stack' || inst.loc?.key !== key) return null;
  let node = expressionOf(valueOf(inst.args?.[0]), values);
  if (!node) return null;
  const bytes = positiveAccessSize(inst.loc?.size ?? inst.size);
  if (bytes == null || expectedSize != null && bytes !== expectedSize) return null;
  const bits = bytes * 8;
...
function committedLocationForPhi(result, value) {
  // Clang's O0 return spill commonly stores a width-preserving MOV view of the
  // branch PHI. Follow only the already validated exact-view chain before
  // asking the predecessor stores for their one committed lvalue.
  const trace = exactViewTrace(value);
  const definition = trace?.root?.def ?? value?.def;
...
function unsafeBarrier(inst, key) {
  ...
  if (inst?.op === 'call') {
    ...
    if (extra?.callCompleteness !== 'complete'
        || typeof extra.semanticNodeId !== 'string' || extra.semanticNodeId.length === 0
        || typeof extra.summarySource !== 'string' || extra.summarySource.length === 0
        || typeof inst.semanticNodeId !== 'string' || inst.semanticNodeId !== extra.semanticNodeId
        || typeof inst.sourceEntityId !== 'string' || inst.sourceEntityId !== inst.semanticNodeId) return true;
    const memoryWrite = extra.memoryWrite;
    if (!memoryWrite || typeof memoryWrite !== 'object' || Array.isArray(memoryWrite)) return true;
    if (!['none', 'accesses', 'all'].includes(memoryWrite.scope)) return true;
    if (memoryWrite.scope === 'none') {
      if (Object.hasOwn(inst, 'memKills') && (!Array.isArray(inst.memKills) || inst.memKills.length !== 0)) return true;
      return false;
    }
    if (!Array.isArray(inst.memKills)) return true;
    return inst.memKills.some((loc) => !loc || typeof loc !== 'object'
      || loc.kind === 'unknown' || !loc.key || loc.key === key);
  }
...
function resolve(ir, blockIndex, beforeRow, key, values, opts, engine, active, depth = 0, expectedSize = null) {
  if (blockIndex == null || depth > 64) return null;
  const token = `${blockIndex}:${beforeRow ?? 'end'}:${key}:${expectedSize ?? 'any'}`;
  if (active.has(token)) return null;
  active.add(token);
  try {
    const memoryByRow = new Map();
    for (const candidate of ir.instructions || []) {
      if (candidate?.block !== blockIndex) continue;
      const descriptor = memoryMutationDescriptor(candidate);
      if (!descriptor) continue;
      const row = memoryOwnData(candidate, 'row');
      if (!row.present || !row.valid || !Number.isSafeInteger(row.value) || row.value < 0) return null;
      const prior = memoryByRow.get(row.value) || [];
      if (prior.some((entry) => memoryMutationCollides(descriptor, entry)
          || memoryMutationCollides(entry, descriptor)
          || (descriptor.op !== 'load' && entry.op !== 'load'
            && (descriptor.broad || entry.broad || descriptor.key === entry.key)))) return null;
      prior.push(descriptor); memoryByRow.set(row.value, prior);
    }
    for (const inst of before(ir, blockIndex, beforeRow)) {
      const stored = storeValue(inst, key, values, expectedSize);
    ...
    const incoming = predecessors.map((pred) => resolve(ir, pred, null, key, values, opts, engine, active, depth + 1, expectedSize));
...
function physicalStackLoadFor(root, ir) {
  if (root?.kind !== 'load' || root.location?.kind !== 'stack' || !root.location?.key) return null;
  const ids = root.source?.ir;
  if (!Array.isArray(ids) || ids.length === 0) return null;
  if (ids.some((id) => !(typeof id === 'number' && Number.isSafeInteger(id) || typeof id === 'string' && id.length > 0))) return null;
  const matches = (ir.instructions || []).filter((instruction) =>
    instruction?.op === 'load' && instruction.loc?.kind === 'stack'
      && instruction.loc.key === root.location.key && ids.some((id) => String(id) === String(instruction.id)));
...
export function recoverExactStackReturn(result, opts = {}) {
  ...
  const deterministic = opts?.deterministicTransforms === true;
  const nodeBudget = numericOption(opts, 'decompilerNodeBudget', (value) =>
    typeof value === 'number' && Number.isSafeInteger(value) && value >= 0);
  const timeBudgetMs = numericOption(opts, 'decompilerTimeBudgetMs', (value) =>
    typeof value === 'number' && Number.isFinite(value) && value >= 0);
  const shouldAbort = callbackOption(opts, 'shouldAbort');
  if (shouldAbort?.() === true) { markRecoveryIncomplete(result, 'recovery-cancelled'); return result; }
  if (nodeBudget === 0 || (timeBudgetMs === 0 && !deterministic)) return result;
  const returnNodes = (result.cAst.body || []).filter((node) =>
    node?.semantic?.op === 'return' || /^return\b/.test(String(node?.text || '').trim()));
  if (returnNodes.length > 1 && !returnNodes.some((node) =>
    node.semantic?.expression?.kind === 'load' && node.semantic.expression.location?.kind === 'stack')) return result;
  const output = result.semanticAst.outputs?.find((x) => x.name === 'return');
  const root = output?.expression;
  if (root?.kind !== 'load' || root.location?.kind !== 'stack' || !root.location?.key) return result;
  const physicalLoad = physicalStackLoadFor(root, result.ir);
  if (!physicalLoad || positiveAccessSize(physicalLoad.loc?.size) * 8 !== root.bits) return result;
  ...
  const engine = new RewriteHistoryJournal(new RewriteEngine(DEFAULT_RULES, {
    maxIterations:10,
    nodeBudget:Math.min(2048, nodeBudget ?? 12000),
    timeBudgetMs:Math.min(12, (timeBudgetMs ?? 50) / 4),
    deterministic,
    maxApplications:512,
  ...
  const recovered = committed || resolve(result.ir, physicalLoad.block, physicalLoad.row, root.location.key, values, opts, engine, new Set(), 0, physicalLoad.loc.size);
...
/**
 * Preserve a committed field value at an intermediate stack spill before a
 * later call. This is a presentation-only recovery: the underlying IR and
 * alias facts remain unchanged, and every candidate must have one exact PHI
 * predecessor store for the same semantic location.
 */
export function recoverCommittedPhiSpillSnapshots(result, opts = {}) {
  if (!result?.semantic || !result.ir || !result.semanticAst || !Array.isArray(result.cAst?.body)) return result;
  const shouldAbort = typeof opts.shouldAbort === 'function' ? opts.shouldAbort : () => false;
  const instructions = Array.isArray(result.ir.instructions) ? result.ir.instructions : [];
  const changes = [];

  for (const store of instructions) {
    if (shouldAbort()) return result;
    if (store?.op !== 'store' || store.loc?.kind !== 'stack' || !store.loc.key) continue;
    const size = positiveAccessSize(store.loc.size);
    const args = store.args;
    if (size == null || !Array.isArray(args) || args.length !== 1) continue;
    const value = valueOf(args[0]);
    const trace = exactViewTrace(value);
    const phi = trace?.root?.def;
    if (phi?.op !== 'phi' || phi.block !== store.block || phi.row == null
        || store.row == null || phi.row > store.row || value?.bits !== size * 8) continue;
    const block = result.ir.blocks?.[phi.block];
    const predecessors = Array.isArray(block?.pred) ? block.pred : [];
    const incoming = Array.isArray(phi.incoming) ? phi.incoming : [];
    const incomingFrom = new Set(incoming.map((item) => item?.from));
    if (!instructions.includes(phi) || incoming.length !== predecessors.length
        || incomingFrom.size !== predecessors.length
        || predecessors.some((pred) => !incomingFrom.has(pred))) continue;

    const location = committedLocationForPhi(result, value);
    if (!location) continue;
    const predecessorStores = incoming.map((item) =>
      storeOfExactValue(result.ir, item?.from, item?.value));
    if (predecessorStores.some((candidate) =>
      !candidate || candidate.loc?.kind === 'stack' || candidate.loc?.kind === 'unknown'
      || !candidate.loc?.key || candidate.loc.key !== location.key)) continue;

    // This pass is intentionally stricter than the normal private-stack
    // return path: any call between the PHI and the spill invalidates the
    // snapshot unless it is outside that interval.
    let safe = true;
    for (const instruction of instructions) {
      if (shouldAbort()) return result;
      if (instruction === store || instruction.block !== store.block) continue;
      if (instruction.row == null) { safe = false; break; }
      if (instruction.row < phi.row || instruction.row > store.row) continue;
      if (instruction.op === 'call' || committedStoreBarrier(instruction, location.key)) {
        safe = false;
        break;
      }
    }
    ...
    const nodes = result.cAst.body.filter((node) => node?.semantic?.op === 'store'
      && String(node.semantic.ir) === String(store.id)
      && node.semantic.location?.key === store.loc.key);
    ...
    const semanticStores = (result.semanticAst.stores || []).filter((item) =>
      item?.location?.key === store.loc.key
      && (item.source?.ir || []).some((id) => String(id) === String(store.id)));
    changes.push({ node, expression,
      stores:semanticStores.map((item) => ({ item, expression:item.expression })),
    });
  }

  if (!changes.length || shouldAbort()) return result;
  const publication = {
    pseudocode:result.pseudocode,
    sourceMap:result.sourceMap,
    lines:result.lines,
    nodes:changes.map(({ node, stores }) => ({
      node, text:node.text, expression:node.semantic.expression,
      stores:stores.map(({ item, expression }) => ({ item, expression })),
    })),
  };
  const rollback = () => {
    result.pseudocode = publication.pseudocode;
    result.sourceMap = publication.sourceMap;
    result.lines = publication.lines;
    for (const prior of publication.nodes) {
      prior.node.text = prior.text;
      prior.node.semantic.expression = prior.expression;
      for (const item of prior.stores) item.item.expression = item.expression;
    }
  };

  try {
    for (const change of changes) {
      change.node.semantic.expression = change.expression;
      for (const prior of change.stores) prior.item.expression = change.expression;
      change.node.text = `${change.node.semantic.location.text} = ${printExpression(change.expression)};`;
    }
    const printed = printProgram(result.cAst, { columnWidth:opts.columnWidth || 88 });
    if (shouldAbort()) { rollback(); return result; }
    result.pseudocode = printed.text;
    result.sourceMap = printed.mapping;
    result.lines = result.cAst.body.map((node) => ({
      kind:node.kind, indent:node.indent, text:node.text,
