...
const selectRules = [{
  ...
  // A boolean condition is a one-bit value, while the materialized arms may
  // intentionally carry the surrounding result width. Preserve that width
  // explicitly instead of returning the narrower condition (#8866).
  precondition: (n) => widthOf(n) != null && widthOf(n.condition) === 1,
  rewrite: (n) => widthOf(n) === 1
    ? n.condition
    : expr.unary('zext', n.condition, widthOf(n), false, n.source, { fromBits:1 }),
  proof: proof('conditional-identity', 'cond ? 1 : 0 with width-preserving materialization'), cost,
}, {
