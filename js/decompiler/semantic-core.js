...
function discardInitialRenderValidation(ctx, reason = 'initial-render-validation-stale') {
  for (const [history, table] of [
    ...
    for (const event of history.events) if (event?.node && table) table.delete(event.node);
...
export function decompileSemantic(model, rawOpts = {}) {
  ...
  // Rendering repeatedly asks whether the same producer observations are still
  // current. Re-walking a large canonical IR for every emitted statement made
  // provenance validation dominate FAST latency. Reuse exact answers only
  // inside this synchronous render section, then revalidate every answer before
  // any history is published. If anything changed, discard all bindings
  // created under the batch; pseudocode itself never gains authority from the
  // memoized validation answer.
