...
function producerBoundExtraDigest(definition, memo, digests) {
  ...
  return {
    ...
    digest: remainder == null ? null : semanticDigest(remainder, memo, digests, '$.definition.extra.remainder'),
  };
}
...
function definitionShape(definition, extraSkip = [], memo = null, digests = null, definitionCache = null) {
  ...
  const producerExtra = producerBoundExtraDigest(definition, memo, digests);
  if (producerExtra) {
    // The exact canonical attributes are already covered once by the producer's
    // authority-bound Semantic IR digest. Hash only mutable/projected wrapper
    // fields here; a replaced attributes reference falls back to the full
    // legacy metadata digest above on the next identity request.
    shape.extraDigest = producerExtra.digest;
    shape.canonicalAttributesNodeId = producerExtra.semanticNodeId;
  } else {
...
function irShape(ir) {
  ...
  try {
    ...
    if (semanticBinding?.semanticIr && typeof semanticBinding.semanticIrDigest === 'string') {
      const nodes = new Map((semanticBinding.semanticIr.nodes ?? []).map((node) => [token(node?.id), node]));
      if (nodes.size === (semanticBinding.semanticIr.nodes ?? []).length) {
