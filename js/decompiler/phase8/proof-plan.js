...
import { committedProofOverlay, registerProofTransactionAdapter } from './transaction.js';
...
let transactionAdapterRegistered = false;

function ensureTransactionAdapter() {
  if (transactionAdapterRegistered) return;
  registerProofTransactionAdapter({
    descriptor: PROOF_REWRITE_PASS,
    admission: proofAdmissionReason,
    publication: proofPublicationResult,
  });
  transactionAdapterRegistered = true;
}

export const PROOF_REWRITE_PASS = createPassDescriptor({
  id:'phase8.solver-constants', version:'2.10.0', stage:'rendering',
...
function targetExclusionReason(target, guard) {
  ...
  while (pending.length) {
    ...
    if (![OP.CONST,OP.MOV,OP.BIN,OP.UN,OP.CMP,OP.SEL,OP.BFX,OP.BFI].includes(def.op)) return unsupported;
