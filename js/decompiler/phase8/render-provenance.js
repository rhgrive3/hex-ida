...
function structuralRole(kind, text) {
  ...
  if (kind === 'ctrl') {
    ...
    return value === '{' || value === '}' || value === '} else {' || value === 'else {' || value === 'do {' || /^loc_\w+:$/.test(value);
  }
  return false;
}

function canonicalScalar(value) {
  // Entity/reverse-map identities stay serializable. Typed address identity is
  // preserved separately in the transform ledger where consumed Source
  // provenance is an authority record rather than an index key.
...
function canonicalList(values, sortNumeric) {
  ...
  for (const value of values ?? []) {
    ...
    if (out.some((existing) => existing === canonical)) continue;
    out.push(canonical);
  }
  out.sort((left, right) => {
...
function originKey(kind, value) { return `${kind}:${String(value)}`; }
...
function originKeySet(origins) { return new Set(entityOriginEntries(origins).map(([kind, value]) => originKey(kind, value))); }
function recordFeedsEntity(record, entityOriginKeys) {
  const recordOrigins = canonicalOrigins(record?.origin ?? {});
  return entityOriginEntries(recordOrigins).some(([kind, value]) => entityOriginKeys.has(originKey(kind, value)));
}
function emptyOrigins() { return { rows:[], addresses:[], ir:[], ssaRefs:[] }; }
function originsTotalSize(origins) { return origins.rows.length + origins.addresses.length + origins.ir.length + origins.ssaRefs.length; }
...
function freezeOrigins(origins) {
  return Object.freeze({
    rows:Object.freeze(origins.rows), addresses:Object.freeze(origins.addresses),
    ir:Object.freeze(origins.ir), ssaRefs:Object.freeze(origins.ssaRefs),
  });
}

function cancelledMap(resolvedBudget) {
  return Object.freeze({
    version:RENDER_PROVENANCE_VERSION, snapshotId:null,
    entities:Object.freeze({}), reverse:Object.freeze({}), ledger:Object.freeze([]), transformCount:0,
    budget:Object.freeze({ ...resolvedBudget, truncated:false, truncatedScopes:Object.freeze([]) }),
    completeness:'incomplete', reasons:Object.freeze(['cancelled']),
    counts:Object.freeze({ entities:0, entitiesTruncated:0, transformRecords:0, ledgerTruncated:0, provenanceLoss:0, structuralEntities:0 }),
  });
}
...
export function buildRenderProvenance({ result, snapshotId = null, budget = null, shouldAbort = null } = {}) {
  ...
  if (rawRecordCount > ledgerRecords.length) {
    ...
    truncatedScopes.push('ledger'); reasons.add('truncated');
  }
  ...
  if (result.lines.length > lineCount) {
    ...
    truncatedScopes.push('entities'); reasons.add('truncated');
  }
  ...
  for (let index = 0; index < lineCount; index += 1) {
    ...
    let origins = canonicalOrigins(sourceOf(line.source));
    ...
    if (originsTotalSize(origins) > resolvedBudget.maxOriginsPerEntity) {
      ...
      entityReasons.push('truncated'); truncatedScopes.push('origins'); reasons.add('truncated'); complete = false;
    }
    if (!complete && structuralRole(line.kind, line.text)) { role = 'structural'; complete = true; }
    if (!complete) { entityReasons.push('provenance-loss'); reasons.add('provenance-loss'); }
    entities[entityKey] = Object.freeze({ entityKey, lineIndex:index, kind:line.kind ?? null, role,
      origins:freezeOrigins(origins), complete, reasons:Object.freeze(entityReasons), recordRefs:Object.freeze(recordRefs) });
  ...
  const reverseObject = {};
  for (const key of [...reverse.keys()].sort((a, b) => a.localeCompare(b, 'en'))) {
    reverseObject[key] = Object.freeze([...reverse.get(key)].sort((a, b) => a.localeCompare(b, 'en')));
  }
  const ledger = ledgerRecords.map((record, recordIndex) => {
    const typed = sourceOf(record.origin);
    return Object.freeze({
      ...record,
      origin:Object.freeze({
        addresses:Object.freeze(canonicalLedgerAddresses(typed.addresses)),
        rows:Object.freeze(canonicalList(typed.rows, true)),
        ir:Object.freeze(canonicalList(typed.ir, false)),
        ssaDefs:Object.freeze(canonicalList(typed.ssaDefs, false)),
        ssaUses:Object.freeze(canonicalList(typed.ssaUses, false)),
      }),
      producedRefs:Object.freeze([...entityRefsByRecord[recordIndex]].sort((a, b) => a.localeCompare(b, 'en'))),
      removedRefs:Object.freeze([]), version:RENDER_PROVENANCE_VERSION,
    });
  });

  if (snapshotId == null) reasons.add('missing-snapshot');
  ...
  return Object.freeze({
    version:RENDER_PROVENANCE_VERSION, snapshotId,
    entities:Object.freeze(entities), reverse:Object.freeze(reverseObject), ledger:Object.freeze(ledger), transformCount:rawRecordCount,
    budget:Object.freeze({ ...resolvedBudget, truncated:truncatedScopes.length > 0, truncatedScopes:Object.freeze(truncatedScopes) }),
    completeness, reasons:Object.freeze([...reasons]),
    counts:Object.freeze({ entities:Object.keys(entities).length, entitiesTruncated, transformRecords:rawRecordCount, ledgerTruncated, provenanceLoss, structuralEntities }),
  });
}
...
export function validateRenderProvenance(provenanceMap, { snapshotId = null, shouldAbort = null } = {}) {
  ...
  if (!validationCancelled) {
    for (const entity of Object.values(provenanceMap.entities)) {
      if (typeof shouldAbort === 'function' && shouldAbort() === true) { reasons.add('cancelled'); validationCancelled = true; break; }
      if (entity?.complete === false) { reasons.add('provenance-loss'); for (const reason of entity.reasons ?? []) reasons.add(reason); }
    }
  }
  ...
  const entityStates = validationCancelled ? [] : entityEntries.slice(0, VALIDATION_ENTITY_STATES_LIMIT).map((entity) => Object.freeze({
    entityKey:entity.entityKey, complete:entity.complete === true, reasons:Object.freeze([...(entity.reasons ?? [])]),
  }));
  return Object.freeze({
    state:reasons.size === 0 ? 'complete' : 'incomplete', entityStates:Object.freeze(entityStates),
    entityStatesTruncated:validationCancelled ? entityEntries.length : Math.max(entityEntries.length - entityStates.length, 0),
    reasons:Object.freeze([...reasons]),
    counts:Object.freeze({ entities:entityEntries.length, transformRecords:provenanceMap.ledger.length,
      provenanceLoss:provenanceMap.counts?.provenanceLoss ?? entityEntries.filter((entity) => entity?.complete === false).length,
