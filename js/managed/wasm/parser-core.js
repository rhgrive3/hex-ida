...
function readExprVector(bytes,offset){const countR=decodeUleb128(bytes,offset);let pos=countR.nextOffset;const values=[];for(let i=0;i<countR.value;i++){const e=readConstExpr(bytes,pos);pos=e.nextOffset;values.push(e.value);}return{value:values,nextOffset:pos};}
...
function readElementSegment(bytes,offset){
  ...
  if(flags===0){const e=readConstExpr(bytes,pos);pos=e.nextOffset;offsetExpr=e.value;}
  ...
  else if(flags===2){const ti=decodeUleb128(bytes,pos);pos=ti.nextOffset;tableIndex=ti.value;const e=readConstExpr(bytes,pos);pos=e.nextOffset;offsetExpr=e.value;const ek=readByte(bytes,pos,'wasm-truncated-element-kind');pos=ek.nextOffset;if(ek.value!==0)fail('wasm-invalid-element-kind');}
  else if(flags===4){const e=readConstExpr(bytes,pos);pos=e.nextOffset;offsetExpr=e.value;}
  ...
  else if(flags===6){const ti=decodeUleb128(bytes,pos);pos=ti.nextOffset;tableIndex=ti.value;const e=readConstExpr(bytes,pos);pos=e.nextOffset;offsetExpr=e.value;const rt=readRefType(bytes,pos,'wasm-truncated-element-ref-type','wasm-invalid-element-ref-type');pos=rt.nextOffset;refType=rt.value;}
  ...
  const vector=legacyForm?readIndexVector(bytes,pos,'wasm-invalid-element-vector'):readExprVector(bytes,pos);
...
function readFunctionInstructionImmediate(bytes,offset,opcode){
  ...
  if(opcode===0xd0){if(offset>=bytes.length)fail('wasm-truncated-ref-null-type');if(WASM_VALUE_TYPES.has(bytes[offset]))return offset+1;return decodeSleb128(bytes,offset).nextOffset;}
  if(opcode===0xfc){const sub=decodeUleb128(bytes,offset);let pos=sub.nextOffset;if(sub.value<=7)return pos;if(sub.value===8||sub.value===10||sub.value===12||sub.value===14)return readFunctionU32(bytes,readFunctionU32(bytes,pos));if(sub.value===9||sub.value===11||sub.value===13||(sub.value>=15&&sub.value<=17))return readFunctionU32(bytes,pos);fail(`wasm-unsupported-fc-opcode-${sub.value}`);}
