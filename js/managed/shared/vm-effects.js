...
export function validateVMEffectBundle(bundle) {
  ...
  if (typeof bundle.schemaVersion !== 'number' || bundle.schemaVersion !== VM_EFFECTS_SCHEMA_VERSION) fail('vm-effect-schema-version-mismatch');
  if (typeof bundle.contractVersion !== 'string' || bundle.contractVersion !== VM_EFFECTS_CONTRACT_VERSION) fail('vm-effect-contract-version-mismatch');
...
export function validateVMEffectFunction(fn) {
  ...
  nonEmpty(fn.methodId, 'vm-effect-function-invalid-structure');
  nonEmpty(fn.frontendId, 'vm-effect-function-invalid-structure');
