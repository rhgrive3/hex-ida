...
function claimMemberSignature(seen,name,descriptor,code){let descriptors=seen.get(name);if(!descriptors){descriptors=new Set();seen.set(name,descriptors);}if(descriptors.has(descriptor))fail(code);descriptors.add(descriptor);}
...
export function parseJvm(bytes,options={}){
  ...
  ensure(pos,2,'jvm-truncated-fields-count');const fieldsCount=view.getUint16(pos,false);pos+=2;const fields=[],seenFieldSignatures=new Map();
  for(let i=0;i<fieldsCount;i++){
    ...
    const field={accessFlags:fFlags,name:requireMemberName(nameIdx,'jvm-invalid-field-name-index'),descriptor:fieldDescriptor};if(constantValue!==null)field.constantValue=constantValue;claimMemberSignature(seenFieldSignatures,field.name,field.descriptor,'jvm-duplicate-field-name-descriptor');fields.push(field);
  }
  ensure(pos,2,'jvm-truncated-methods-count');const methodsCount=view.getUint16(pos,false);pos+=2;const methods=[],seenMethodSignatures=new Map();
