...
export function parseJvm(bytes,options={}){
  ...
  function validateConstantPool(){for(let i=1;i<constantPool.length;i++){const entry=constantPool[i];if(!entry)continue;switch(entry.tag){
    ...
    // never resolved.
    if(memberName==='<init>'){const parsed=parseJvmMethodDescriptor(requireUtf8(nameAndType.descriptorIndex,'jvm-invalid-cp-memberref-name-and-type-index-descriptor'));if(parsed.returnType!==null)fail('jvm-invalid-cp-methodref-init-return-type');}else if(memberName.startsWith('<'))fail('jvm-invalid-cp-methodref-clinit-name');}break;}
