...
export function parseJvm(bytes,options={}){
  ...
  // JVMS §4.6 method_info.access_flags: at most one of PUBLIC/PRIVATE/
  // PROTECTED (package-private none is legal), and ACC_ABSTRACT excludes
  // PRIVATE/STATIC/FINAL/SYNCHRONIZED/NATIVE/STRICT. The JVM rejects these
  // combinations with ClassFormatError, so the parser must too (#7264).
  function requireValidMethodFlags(flags){
    const visibility=flags&(0x0001|0x0002|0x0004);
    if(visibility!==0&&visibility!==0x0001&&visibility!==0x0002&&visibility!==0x0004)fail('jvm-method-visibility-conflict');
    if((flags&0x0400)!==0&&(flags&(0x0002|0x0008|0x0010|0x0020|0x0100|0x0800))!==0)fail('jvm-method-abstract-flag-conflict');
  }
  ...
  for(let i=0;i<methodsCount;i++){ensure(pos,8,'jvm-truncated-method-info');const mFlags=view.getUint16(pos,false),nameIdx=view.getUint16(pos+2,false),descIdx=view.getUint16(pos+4,false),attrCount=view.getUint16(pos+6,false);pos+=8;let codeAttr=null,codeCount=0;requireValidMethodFlags(mFlags);
