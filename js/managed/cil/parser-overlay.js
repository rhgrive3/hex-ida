...
import { metadataRowSize, validateMetadataTableValidMask, createCilMetadataBudget } from './metadata-layout.js';
...
export function overlayCilMetadata(bytes,parsed,options={}){
 ...
 const layout=tableLayout(u8,view,tablesStream),defs=bindCilMetadataTables(readCilDefinitions(u8,view,layout,stringsStream,blobStream,guidStream,budget),blobHeap),genericMetadata=readCilGenericMetadata(u8,view,layout,stringsStream,defs,budget);validateParamAuthority(u8,defs,layout,blobStream);const properties=decodePropertyAuthority(u8,defs,layout,blobStream);validateManagedEntryAuthority(u8,parsed,defs,layout,meta);const byOffset=new Map((parsed.methodBodies??[]).map(b=>[b.headerOffset,b])),methodBodies=[],methods=[];
