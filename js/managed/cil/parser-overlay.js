...
export function overlayCilMetadata(bytes,parsed){
 ...
 const layout=tableLayout(u8,view,tablesStream),defs=readCilDefinitions(u8,view,layout,stringsStream,blobHeap);validateParamAuthority(u8,defs,layout,blobStream);const properties=decodePropertyAuthority(u8,defs,layout,blobStream);validateManagedEntryAuthority(u8,parsed,defs,layout,meta);const byOffset=new Map((parsed.methodBodies??[]).map(b=>[b.headerOffset,b])),methodBodies=[],methods=[];
