// One bounded metadata-layout authority for every CIL reader. The format probe,
// the canonical overlay and the manifest/security overlay previously each
// re-derived the layout and re-decoded the complete definition graph from the
// same bytes, multiplying the exact work an untrusted image can force (#8704).
...
export function readCilTableLayout(bytes, view, stream) {
...
// Validate the layout once, admit every declared row against the aggregate
// budget, and decode the canonical definition graph exactly one time.
export function readCilMetadataContext(bytes, options = {}) {
  const u8 = asUint8Array(bytes), view = new DataView(u8.buffer, u8.byteOffset, u8.byteLength);
  const pe = readCilPeLayout(u8, view);
  if (!pe?.cliPresent) return null;
  const meta = readCilMetadataStreams(u8, pe.metadataOffset, pe.metadataSize);
  const find = name => meta.streams.find(s => s.name === name);
  const tablesStream = meta.streams.find(s => s.name === '#~' || s.name === '#-');
  const stringsStream = find('#Strings'), blobStream = find('#Blob'), guidStream = find('#GUID'), usStream = find('#US');
  if (!tablesStream) fail('cil-metadata-tables-missing');
  const layout = readCilTableLayout(u8, view, tablesStream);
  const admission = options.metadataAdmission ?? createCilMetadataAdmission(options.metadataBudget ?? {});
  admitCilMetadataTables(admission, layout);
  const blobHeap = blobStream ? u8.subarray(blobStream.offset, blobStream.offset + blobStream.size) : null;
  const defs = bindCilMetadataTables(
    readCilDefinitions(u8, view, layout, stringsStream, blobStream, guidStream, admission),
    blobHeap,
    admission,
  );
  return {
    version: CIL_METADATA_CONTEXT_VERSION,
    bytes: u8,
    view,
    pe,
    meta,
