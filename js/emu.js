...
export class Emulator {
  ...
  async step(options = {}) {
    ...
    this.pc = next;
    if (this.pc === 0n) { this.stopped = '最初の呼び出し元まで戻ってきました（実行おわり）。'; this.stopCode = 'program-return'; }
  ...
  async execute(mn, opsStr, at) {
    ...
    if (Object.prototype.hasOwnProperty.call(arith, mn)) {
      ...
      const a = R(ops[1]);
      const b = R(ops[2]);
      const variableShift = b & (wide ? 63n : 31n);
      let r;
      if (mn === 'lsl') r = a << variableShift;
      else if (mn === 'lsr') r = (wide ? a : a & MASK32) >> variableShift;
      else if (mn === 'asr') r = BigInt.asIntN(wide ? 64 : 32, a) >> variableShift;
