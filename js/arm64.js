...
function mteTagHandler({ isLoad, zeroing = false, span = 1 }) {
  return (o, ops) => {
    ...
    if (isLoad) {
      ...
      o.pseudo = opShort(dst) + ' = AllocationTag(' + addrExpr + ') ' +
        J('/* タグを運ぶ。データビットはゼロ */', '/* carried tag; data bits zeroed */');
      o.summary = J(
        spanJa + ' に付いた Allocation Tag を読み、' + opShort(dst) + ' にタグを運ばせたポインタを作ります。通常のデータ値を読む命令ではありません。',
        'Read the Allocation Tag of ' + spanEn + ' and build a pointer in ' + opShort(dst) + ' carrying that tag; its data address bits read as zero. This is not an ordinary data load.');
    } else {
