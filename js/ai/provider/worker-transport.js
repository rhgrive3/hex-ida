...
export async function waitForRetry(attempt, retryAfter, signal) { const delay = retryDelayMs(attempt, retryAfter); if (signal.aborted) return false; if (delay <= 0) return true; return new Promise((resolve) => { let settled = false; const finish = (value) => { if (settled) return; settled = true; clearTimeout(timer); signal.removeEventListener('abort', onAbort); resolve(value); }; const onAbort = () => finish(false); const timer = setTimeout(() => finish(true), delay); signal.addEventListener('abort', onAbort, { once: true }); if (signal.aborted) onAbort(); }); }
