...
import { CapabilityExecutor as MainCapabilityExecutor } from './executor-base-main.js';

export * from './executor-base-main.js';
...
export class CapabilityExecutor extends MainCapabilityExecutor {
  ...
  async approvalState(id, args = {}, runtimePlatform = null) {
    ...
    switch (id) {
      ...
      case 'runtime.memory-write': {
        ...
        const bytes = await runtimeAdapter(platform).readMemory(executionArgs.address, expected.length);
...
function captureMutationContext(entry, app, runtimePlatform) {
  const patchBound = entry?.id?.startsWith?.('patch.') || entry?.id === 'annotation.project';
  const session = entry?.category === 'runtime' ? runtimePlatform?.currentSession?.(false) : null;
  return Object.freeze({
    patchSet: patchBound ? app?.patches || null : null,
...
function assertMutationContext(context, app, runtimePlatform) {
  if (context?.patchSet && app?.patches !== context.patchSet) throw new AIError('scope_violation', 'The approved proposal mutation target changed before commit.');
...
function snapshotApprovedArguments(value) {
  ...
  try { return structuredClone(value); } catch { throw new AIError('invalid_tool_call', 'Approved capability arguments must be structured-cloneable.'); }
}
function runtimeAdapter(platform) {
  const session = platform?.currentSession?.();
...
function runtimeStatus(platform) {
  ...
  return session ? { connected: !!session.adapter?.connected, sessionId: session.id, binaryId: session.binaryHash || null, backend: session.backend, capabilities: session.adapter?.capabilities || {} } : { connected: false, sessionId: null };
}
async function guardedCreatePatch(app, args, commitGuard) {
  ...
  commitGuard?.();
  app.patches?.add?.(range.fileOffset, before, after, { addr: address, label: args.label || null, reason: args.reason || null, expectedBefore: before, createdAt: new Date().toISOString() });
...
function serializePatch(item) {
  return { fileOffset: item.offset.toString(), address: item.addr == null ? null : String(item.addr), before: Array.from(item.before), after: Array.from(item.after), label: item.label || null, reason: item.reason || null };
}
