...
function sealOwnedConfirmedFindings(session) {
  if (Array.isArray(session?.confirmedFindings)) {
    sealPersistedConfirmedEnvelope(session.confirmedFindings);
  }
  return session;
}
...
export class InvestigationSessionStore {
  ...
  register(session) {
    ...
    const validated = sealOwnedConfirmedFindings(freezeOwned(createInvestigationSession(session)));
  ...
  async create(input) {
    ...
    const session = sealOwnedConfirmedFindings(freezeOwned(createInvestigationSession(input)));
    const id = session.id;
  ...
  async get(id) {
    ...
    if (this.persistence && typeof this.persistence.load === 'function') {
      ...
      if (loaded) {
        ...
        const owned = sealOwnedConfirmedFindings(freezeOwned(session));
  ...
  async applyUpdate(id, patch = {}) {
    ...
    const ownedCandidate = sealOwnedConfirmedFindings(freezeOwned(createInvestigationSession(candidate)));
