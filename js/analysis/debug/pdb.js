...
export function parseTpiStream(bytes, budget = DEBUG_DEFAULT_BUDGET) {
  ...
  // Complete only when the declared record extent was fully consumed, the
  // declared index window is sane, and the parsed record count matches
  // TypeIndexEnd - TypeIndexBegin. Trailing bytes beyond TypeRecordBytes
  // (e.g. hash data) are not type records and do not block completeness.
  return { types, unmodelled, complete: expectedCount >= 0 && offset >= typeDataEnd && index - firstIndex === expectedCount, firstIndex };
}
