...
export function createPointsToTarget(input = {}) {
  ...
  const target = {
    // Address spaces are identity strings, not free-form labels: non-canonical
    // spelling (e.g. stray whitespace) must not become a fake separation proof
    // downstream (#5717), so they are canonicalized here exactly like the
    // regions-v2 boundary does.
    addressSpace: input.addressSpace == null
      ? 'memory'
      : (typeof input.addressSpace === 'string' && input.addressSpace.trim() ? input.addressSpace.trim() : 'unknown'),
