...
export function createDiscoveryEvidence(input = {}, overrides = {}) {
  ...
  return deepFreeze({
    ...
    regions: deepFreeze(regions.map((region) => createRegion(region)).sort(compareCanonicalRegions)),
    ...
    architectureId: optionalArchitectureId(snapshot.architectureId),
...
export function createFunctionCandidate(input = {}) {
  ...
  const regions = dataArray(snapshot.regions ?? [], 'discovery-candidate-invalid-regions').map((region) => createRegion(region));
  ...
  const architectureId = snapshot.architectureId == null
    ? null : optionalArchitectureId(snapshot.architectureId);

  const startEvidence = deepFreeze(dataArray(snapshot.startEvidence ?? [], 'discovery-candidate-start-evidence-invalid')
  ...
  const candidate = {
    ...
    startState,
    extentState,
    conflicts: deepFreeze(candidateConflicts(dataArray(snapshot.conflicts ?? [], 'discovery-candidate-invalid-conflicts'))
