...
export function reconcileCandidateOverlaps(candidates, { signal = null } = {}) {
  ...
  for (const region of regions) {
    ...
    for (let index = left; index < starts.length && starts[index].start < region.end; index += 1) {
      ...
      if (other !== region.candidateIndex) {
        if (region.ownership === 'shared' && sharedAtStart[other]) continue;
        swallowed[region.candidateIndex].push(candidates[other].start);
      }
    }
  }
  ...
  for (const event of events) {
    ...
    for (const item of active) {
      ...
      if (item.ownership === 'shared' && event.region.ownership === 'shared') continue;
