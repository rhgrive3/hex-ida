...
export function createDiscoveryArtifact(input = {}) {
  ...
  // Identical evidence can be emitted more than once by a producer. Preserve
  // that multiplicity in `evidence`, while treating the canonical interval
  // claim as a set so duplicate claims cannot make artifact construction fail.
  const intervalById = new Map();
  for (const item of [...supplied, ...inferred]) intervalById.set(item.intervalId, item);
  const intervals = [...intervalById.values()].sort(compareInterval);
...
function rebuildBindingIdentityValid(binding) {
  try {
    if (!binding || typeof binding !== 'object' || Array.isArray(binding)
        || binding.schemaVersion !== DISCOVERY_REBUILD_BINDING_SCHEMA
        || typeof binding.digest !== 'string') return false;
    ...
    return binding.digest === stableDigest(payload)
      ...
      && Array.isArray(binding.functionCandidates)
      && Array.isArray(binding.intervalClaims)
      && record(binding.binding, 'discovery-rebuild-binding-identity-invalid');
  } catch {
    return false;
  }
}
...
/**
 * Validate the detached identity carried by a rebuild transaction.
 *
 * The factory-issued check below remains the authority for a live source
 * binding.  This structural predicate is intentionally separate so a
 * transaction can be inspected after persistence or transport without
 * relying on an in-memory WeakSet.
 */
export function isDiscoveryRebuildBinding(binding) {
  return rebuildBindingIdentityValid(binding);
}
...
export function verifyDiscoveryReparse(sourceBinding, reparsedArtifact, options = {}) {
  ...
  // Rebuild cannot invent stronger extent/start claims even when no collision
  // exists. Candidate digests contain source identity, so compare the actual
  // conservative projection rather than hashes that legitimately change when
  // the output bytes acquire a new sourceHash.
  const projection = (item) => ({
    candidateId: item.candidateId, start: item.start, name: item.name,
    startState: item.startState, extentState: item.extentState, exact: item.exact,
    collisionIds: item.collisionIds, startEvidenceIds: item.startEvidenceIds,
  });
  const candidatesPreserved = canonicalEqual(
    sourceBinding.functionCandidates.map(projection), reparsedArtifact.functionCandidates.map(projection),
  );
  const intervalsPreserved = canonicalEqual(sourceBinding.intervalClaims, reparsedArtifact.intervalClaims);
  const ok = missingCollisionIds.length === 0 && missingReferenceIds.length === 0
    && candidatesPreserved && intervalsPreserved;
  return deepFreeze({ ok, reason: ok ? null : 'discovery-reparse-ambiguity-lost',
    missingCollisionIds, missingReferenceIds, candidatesPreserved, intervalsPreserved });
}
