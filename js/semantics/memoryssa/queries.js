...
// Exact facts are capabilities published by this canonical query. Keeping a
// private binding lets downstream predicates reject shape-compatible clones or
// re-signed payloads without creating a second semantic owner in each consumer.
const forwardingFactBindings = new WeakMap();
...
function forwardingProducerRange(region, memory, metadata, options) {
  ...
  if (sourceNode) {
    ...
    forwardingCheckSourceNode(sourceNode, sourceNode.memory, metadata?.sourceKind ?? 'access', { requireComplete: true });
    if (!forwardingObject(sourceNode.memory)
        || stableStringify(sourceNode.memory) !== stableStringify(memory)) return null;
  ...
  const displacement = forwardingBigInt(
  ...
  if (sourceNode) {
    const normalizedDisplacement = displacement
      ?? (sourceNode.attributes?.machineEffects?.operationMetadata?.addressing?.addressDisplacement == null ? 0n : null);
...
function forwardingValueForDefinition(definition, metadata, memorySsa, options) {
  ...
  const parsedValue = forwardingValueFromRaw(proof?.value);
  if (!forwardingObject(proof)
      ...
      || !['address', 'bitvector'].includes(String(proof.valueKind ?? ''))
      // Address operands are identity-only.  Read the candidate before this
      // check, so an inherited/prototype `value` cannot be reinterpreted as a
      // numeric byte constant at the query boundary.
      || (String(proof.valueKind) === 'address'
        && (Object.hasOwn(proof, 'value') || parsedValue != null))
      || (Object.hasOwn(proof, 'value') && parsedValue == null)
      || (parsedValue == null && String(proof.valueKind) !== 'address')
  ...
  if (options.ir != null) {
    ...
    if (!storeNode || storeNode.kind !== 'store' || !Array.isArray(storeNode.inputs)
        ...
        || (parsedValue != null && (semanticValue.metadata?.constant?.value == null
...
function forwardingStatusForReason(raw) {
  ...
  if (/deadline|timeout|budget|limit|unknown|incomplete|missing|widen|partial|exhaust/.test(reason)) {
    // Unknown/partial stop reasons are deliberately reported as a bounded
    // non-exact result.  Keeping the budget-limited status for an ambiguous
    // stop preserves the fail-closed publication contract: it can never be
...
function forwardingStatusFromArtifact(memorySsa, options) {
  ...
  const rawTopLevelStop = forwardingStatusForReason(artifact.stopReason);
  const topLevelStop = rawTopLevelStop === 'unknown' || rawTopLevelStop === 'partial'
    ? 'budget-limited' : rawTopLevelStop;
  ...
  for (const unknown of artifact.unknowns) {
    ...
    if (forwardingMemoryUnknown(unknown)) {
      throw new ForwardingStop('unknown', 'memoryssa-memory-unknown-present');
    }
  }
  ...
  if (artifact.truncated === true || artifact.status?.stopReason === 'budget-exhausted') {
  ...
  if (options.identity != null && stableDigest(artifact.identity ?? null) !== stableDigest(options.identity)) {
    throw new ForwardingStop('stale', 'memoryssa-identity-mismatch');
  }
  if (artifact.identity != null && options.expectedIdentity != null
      && stableDigest(artifact.identity) !== stableDigest(options.expectedIdentity)) {
    throw new ForwardingStop('stale', 'memoryssa-identity-mismatch');
  }
...
function forwardingIdentity(memorySsa, use, loadMeta, winners, stores, coverage, context, options) {
  return {
    digest: stableDigest({
      ...
      optionsIdentity: options.identity ?? options.expectedIdentity ?? null,
...
function forwardingRegisterExactFact(fact, artifact, use, context) {
  ...
  forwardingFactBindings.set(fact, Object.freeze({
    artifact,
    useId: String(use.id),
...
function forwardingFactBindingIsCurrent(fact) {
  const binding = forwardingFactBindings.get(fact);
  ...
  try {
    if (!forwardingObject(artifact)
        ...
        || String(fact.identity?.digest ?? '') !== binding.identityDigest) return false;
...
export function isCanonicalExactMemoryForwarding(fact) {
  ...
  const value = forwardingValueFromRaw(fact.value);
  ...
  return forwardingFactBindingIsCurrent(fact);
}
 ...
 */
export function isCanonicalExactMemoryOperandForwarding(fact) {
  if (!forwardingObject(fact)
      || fact.status !== FORWARD_EXACT
      || fact.exact !== true
      || fact.reason !== null
      ...
      || !forwardingObject(fact.identity)
      || typeof fact.identity.digest !== 'string' || !fact.identity.digest.trim()
      || !['address', 'bitvector'].includes(String(fact.operandKind ?? ''))
...
export function forwardMemoryValue(memorySsa, useOrId, options = {}) {
  ...
  try {
    ...
    const artifact = validated ?? memorySsa;
