...
export function projectSemanticIrV2ToLegacyV1(input, options = {}) {
  ...
  // Compatibility must be able to inspect a structurally valid intrinsic
  // whose data arity is not representable in v1. The node projector then
  // emits an explicit clobber/unknown boundary; it must not borrow the pure
  // add-with-carry lowering for that malformed extension.
  const ir = validateSemanticIrFunction(input, {
    ...(options.validationOptions || {}),
    allowCompatibilityIntrinsicArity: true,
  });
  ...
  for (const block of legacyBlocks) {
    ...
    for (const nodeId of block.semanticNodeIds) {
      ...
      const instructions = projectNode(node, {
        ...
        blockBySemanticId,
        options: { ...options, allowCompatibilityIntrinsicArity: true },
      });
