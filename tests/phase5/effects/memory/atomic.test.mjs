...
test('LOCK BTS remains conservative until atomic bit-string semantics land',()=>{const{bundle}=lift({family:'bts',prefixes:[0xf0],operands:[mem({base:'rax',widthBits:64,access:'read-write'}),reg('rcx',64,'read')]});assert.equal(bundle.completeness,'partial');assert.equal(bundle.metadata.lockIgnored,false);assert.match(bundle.unknownEffects.reason,/lock-prefixed-family-not-modelled/);});
test('memory bit-string address adjustment remains fail-closed without LOCK',()=>{const{bundle}=lift({family:'bts',operands:[mem({base:'rax',widthBits:64,access:'read-write'}),reg('rcx',64,'read')]});assert.equal(bundle.completeness,'partial');assert.equal(bundle.metadata.bitStringAddressing,true);assert.match(bundle.unknownEffects.reason,/memory-bit-string-address-adjustment-unmodelled/);assert.equal(writes(bundle).length,0,'CF must never be misused as the full-width memory result');});
test('invalid LOCK register bit-test form is not treated as an ordinary intrinsic',()=>{const{bundle}=lift({family:'bts',prefixes:[0xf0],operands:[reg('rax',64,'read-write'),reg('rcx',64,'read')]});assert.equal(bundle.completeness,'partial');assert.equal(bundle.metadata.lockIgnored,false);assert.match(bundle.unknownEffects.reason,/lock-prefix-without-memory-operand/);});
test('generic bit-manipulation ownership does not invent exact operand or flag roles',()=>{for(const input of[
  {family:'movbe',operands:[mem({base:'rax',widthBits:32,access:'write'}),reg('ecx',32,'read')]},
  {family:'rcl',operands:[reg('rax',64,'read-write'),reg('cl',8,'read')]},
  {family:'lzcnt',operands:[reg('rax',64,'write'),reg('rcx',64,'read')]},
  {family:'enter',operands:[]},
]){const{bundle}=lift(input);assert.equal(bundle.completeness,'partial',input.family);assert.equal(bundle.metadata.exactArchitecturalSummary,false,input.family);assert.match(bundle.unknownEffects.reason,/requires-dedicated-semantics/,input.family);}});
