...
test('real decoder bytes preserve legacy/VEX structure before P5-3 effects', async () => {
  ...
  try{
    ...
    const fenceRaw=decoder.decode([0x0f,0xae,0xf0],0x401800n)[0],fence=createX86DecodedInstruction({...fenceRaw,instructionId:'p5-3:real:mfence'}); assert.equal(fence.instructionFamily,'mfence'); const fenceBundle=effects(fenceRaw); assert.equal(fenceBundle.metadata.operation,'mfence'); if(fenceBundle.completeness==='exact'){assert.equal(fenceBundle.metadata.exactBarrierContract,true); assert.equal(ops(fenceBundle,'barrier').length,1);}else{assert.equal(fenceBundle.completeness,'partial'); assert.equal(fenceBundle.metadata.barrierOperationEmitted,false);}
