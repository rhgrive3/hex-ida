...
test('#7160 a transiently unreadable delta that repaired stays live after recovery', async () => {
  ...
  await withStorage(harness.storage, () => {
    ...
    assert.equal(probe.lastSaveError?.reloadRequired, true);
    assert.ok(probe.lastSaveError?.pendingKeys?.includes(deltaKey));

    // Recovery must not make this instance eligible for compaction: the
    // repaired record still needs a fresh ordered reload.
    assert.equal(probe.save(), false, 'compaction stays blocked until a fresh reload');
    assert.equal(probe.lastSaveError?.code, 'DELTA_SCAN_INCOMPLETE');
...
test('#7160 complete delta evidence can rebuild a missing or legacy base', async () => {
  await withStorage((() => {
    const map = new Map();
    return {
      get length() { return map.size; },
    ...
    const missingId = 'recovery-7160-missing-base';
    const missingDeltaKey = 'hex.notes.' + missingId + '.delta.comments.8192';
    const missingRaw = JSON.stringify({
      kind: 'comments', key: '8192', deleted: false,
      generation: 'orphan-generation', value: 'recovered from delta',
    });
    globalThis.localStorage.setItem(missingDeltaKey, missingRaw);
    const missing = new NoteStore(missingId);
    assert.equal(missing._deltaScanComplete, false);
    assert.equal(missing._unreadableDeltaKeys.has(missingDeltaKey), true);
    assert.equal(missing.recoverUnreadableDeltas(), true,
      'complete delta evidence can be explicitly adopted when the base is missing');
    assert.equal(missing.comment(0x2000n), 'recovered from delta');
    assert.equal(missing.save(), true);
    assert.equal(globalThis.localStorage.getItem(missingDeltaKey), null);
    assert.equal(new NoteStore(missingId).comment(0x2000n), 'recovered from delta');

    const legacyId = 'recovery-7160-legacy-base';
    const legacyKey = 'hex.notes.' + legacyId;
    const legacyDeltaKey = legacyKey + '.delta.comments.8192';
    globalThis.localStorage.setItem(legacyKey, JSON.stringify({ v: 2, names: {} }));
    const legacyRaw = JSON.stringify({
      kind: 'comments', key: '8192', deleted: false,
      generation: 'migrated-generation', value: 'migrated overlay',
    });
    globalThis.localStorage.setItem(legacyDeltaKey, legacyRaw);
    const legacy = new NoteStore(legacyId);
    assert.equal(legacy._deltaScanComplete, false);
    assert.equal(legacy._unreadableDeltaKeys.has(legacyDeltaKey), true);
    assert.equal(legacy.recoverUnreadableDeltas(), true,
      'an unbound generation can be adopted only through explicit recovery');
    assert.equal(legacy.comment(0x2000n), 'migrated overlay');
    assert.equal(legacy.save(), true);
    assert.equal(globalThis.localStorage.getItem(legacyDeltaKey), null);
    assert.equal(new NoteStore(legacyId).comment(0x2000n), 'migrated overlay');
  });
});
