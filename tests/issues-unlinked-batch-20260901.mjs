...
// #3269 + #3264 — Rust metadata completeness and v0 trailing bytes.
{
  ...
  const vendorSuffix = demangleRustV0('_RNvC3foo3bar.llvm.123');
  assert.equal(vendorSuffix.parsed, true);
  assert.equal(vendorSuffix.demangled, 'foo::bar');
...
// #3263 — Analysis Query artifactVersions structured identity laundering.
{
  ...
  assert.equal(primitive.artifactVersions.architecture, 'primitive:string:5:arm64');
  assert.notEqual(structured.artifactVersions.architecture, primitive.artifactVersions.architecture);
