...
// so structured metadata could launder into canonical primitive identity.
...
test('#5146 structured import metadata never merges into a canonical import', () => {
  const imports = finalizeWith(
    {
      library: 'libA', name: 'foo', ordinal: 1, weak: false,
      address: 0x2000n, source: 'chained-fixups',
      sites: [{ address: 0x1000n, kind: 'got' }],
    },
    {
      library: ['libA'], name: ['foo'], ordinal: ['1'], weak: false,
      address: 0x3000n, source: 'structured-metadata',
...
test('#5146 every structured identity field stays out of the canonical import key', () => {
  ...
  for (const field of fields) {
    ...
    structured[field] = field === 'library' || field === 'name' || field === 'versionLibrary'
      ? ['L'] : ['3'];
...
test('#5146 structured site identity never dedupes into a canonical site', () => {
  const imports = finalizeWith({
    ...
    sites: [
      ...
      { address: [1n], offset: [0n], kind: ['bind'], type: [1], addend: [0n], pointerFormat: [1], weak: false },
      { address: launders('1'), offset: launders('0'), kind: launders('bind'), type: launders('1'), addend: launders('0'), pointerFormat: launders('1'), weak: false },
    ],
  });
...
test('#5146 valid duplicate imports and sites keep merging', () => {
  const imports = finalizeWith(
    {
      library: 'L', name: 'f', ordinal: 1n, weak: false, addend: 0n,
      pointerFormat: 2, version: 'v1', versionLibrary: 'L',
      sites: [{ address: 1n, offset: 1n, kind: 'bind', type: 1, addend: 0n, pointerFormat: 2, weak: false }],
    },
    {
      library: 'L', name: 'f', ordinal: 1n, weak: false, addend: 0n,
      pointerFormat: 2, version: 'v1', versionLibrary: 'L', source: 'elf-dynsym', address: 0x4000n,
      sites: [
...
test('#5146 distinct valid imports are never merged', () => {
  const imports = finalizeWith(
    ...
    { library: 'L', name: 'f', ordinal: 1, version: 2, sites: [{ address: 1n, kind: 'bind' }] },
