...
const fallback = resolveSelectorStub({
  address: 16,
  symbolFor: true,
  selectorFor: {},
  selectorIndex: validIndex,
});
assert.equal(fallback.selector, 'indexed:');
assert.equal(fallback.ambiguous, false);

let symbolCalls = 0;
const symbolHook = resolveSelectorStub({
  address: 64,
  symbolFor(address) {
    symbolCalls++;
    assert.equal(address, 64);
    return '_objc_msgSend$symbolHook:';
  },
});
...
let selectorCalls = 0;
const selectorHook = resolveSelectorStub({
  address: 80,
  selectorFor(address) {
    selectorCalls++;
    assert.equal(address, 80);
    return 'selectorHook:';
  },
});
