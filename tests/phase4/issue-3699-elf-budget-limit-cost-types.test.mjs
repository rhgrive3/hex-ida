...
const preexistingBudgetImage = image();
preexistingBudgetImage.metadata.elfMetadata = { complete:false, reasons:['budget:prior-parser'] };
const freshBudget = createELFMetadataBudget(preexistingBudgetImage, { limits:{ records:1, wallClockMs:10_000 } });
assert.equal(freshBudget.stopped, false, 'a new budget must not inherit a prior parser stop state');
assert.equal(freshBudget.take({ records:1 }, 'fresh-budget'), true);
assert.equal(freshBudget.stopped, false, 'a successful fresh budget must remain usable');
