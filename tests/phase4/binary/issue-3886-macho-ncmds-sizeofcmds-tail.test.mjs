...
{
  const image = parseMachO(macho64({ ncmds: 1, sizeofcmds: 8, commands: [{ cmdsize: 8 }] }));
  assert.equal(image.metadata.machoMetadata.complete, true);
  assert.ok(!reasons(image).includes('load-command-count-size-mismatch'));
}

{
  const image = parseMachO(macho64({ ncmds: 1, sizeofcmds: 16, commands: [{ cmdsize: 8 }] }));
...
{
  const image = parseMachO(macho64({ ncmds: 2, sizeofcmds: 16, commands: [{ cmdsize: 8 }, { cmdsize: 8 }] }));
  assert.equal(image.metadata.machoMetadata.complete, true);
  assert.ok(!reasons(image).includes('load-command-count-size-mismatch'));
}

{
  const image = parseMachO(macho64({ ncmds: 2, sizeofcmds: 8, commands: [{ cmdsize: 8 }, { cmdsize: 8 }] }));
  assert.equal(image.metadata.machoMetadata.complete, false);
  assert.ok(reasons(image).includes('load-command-truncated'));
  assert.ok(!reasons(image).includes('load-command-count-size-mismatch'));
}

{
  const image = parseMachO(macho64({ ncmds: 1, sizeofcmds: 8, commands: [{ cmdsize: 16 }] }));
  assert.equal(image.metadata.machoMetadata.complete, false);
  assert.ok(reasons(image).includes('load-command-invalid-size'));
  assert.ok(!reasons(image).includes('load-command-count-size-mismatch'));
}

console.log('issue-3886-macho-ncmds-sizeofcmds-tail: PASS');
