...
function setSection(view, bits, shoff, index, {
  type = SHT_NULL,
  flags = 0n,
  offset = 0,
  size = 0,
  ...
  if (bits === 64) {
    ...
    view.setBigUint64(p + 8, BigInt(flags), true);
    ...
    view.setUint32(p + 16, offset, true);
    view.setUint32(p + 20, size, true);
    view.setUint32(p + 24, link, true);
    view.setUint32(p + 28, info, true);
    view.setUint32(p + 32, align, true);
    view.setUint32(p + 36, entsize, true);
  }
}

function writeSymbolTable(view, bits, offset) {
  const ent = bits === 64 ? 24 : 16;
  const p = offset + ent; // entry 0 is the ELF null symbol
  if (bits === 64) {
    view.setUint32(p, 1, true);      // st_name -> "ext"
    view.setUint8(p + 4, 0x11);     // STB_GLOBAL | STT_OBJECT
    view.setUint16(p + 6, 0, true); // SHN_UNDEF
  } else {
    view.setUint32(p, 1, true);
    view.setUint8(p + 12, 0x11);
...
function buildSymbolTableELF(bits, remainder) {
  const strOffset = 0x80;
  const symOffset = 0x100;
  const shoff = 0x300;
  const shnum = 3;
  const shsize = bits === 64 ? 64 : 40;
  const bytes = new Uint8Array(shoff + shnum * shsize);
  const view = new DataView(bytes.buffer);
  setHeader(view, bits, { type: ET_REL, shoff, shnum });
  bytes.set(new TextEncoder().encode('\0ext\0'), strOffset);
  const ent = writeSymbolTable(view, bits, symOffset);
  if (remainder) bytes[symOffset + 2 * ent] = 0xa5;
  setSection(view, bits, shoff, 0);
  setSection(view, bits, shoff, 1, { type: SHT_STRTAB, offset: strOffset, size: 5 });
  setSection(view, bits, shoff, 2, {
    type: SHT_SYMTAB,
    offset: symOffset,
    size: 2 * ent + remainder,
    link: 1,
    align: bits === 64 ? 8 : 4,
    entsize: ent,
  });
...
function buildRelocationELF(bits, remainder) {
  const strOffset = 0x80;
  const symOffset = 0x100;
  const textOffset = 0x180;
  const relocOffset = 0x1c0;
  const shoff = 0x300;
  const shnum = 5;
  const shsize = bits === 64 ? 64 : 40;
  const bytes = new Uint8Array(shoff + shnum * shsize);
  ...
  setHeader(view, bits, { type: ET_REL, shoff, shnum });
  bytes.set(new TextEncoder().encode('\0ext\0'), strOffset);
  const symEnt = writeSymbolTable(view, bits, symOffset);
  bytes.fill(0x90, textOffset, textOffset + 16);
  const relocEnt = bits === 64 ? 24 : 8;
  if (bits === 64) {
    view.setBigUint64(relocOffset, 0n, true);
    view.setBigUint64(relocOffset + 8, 2n, true); // sym=0, R_X86_64_PC32
    view.setBigInt64(relocOffset + 16, 0n, true);
  } else {
    view.setUint32(relocOffset, 0, true);
    view.setUint32(relocOffset + 4, 1, true); // sym=0, R_386_32
  }
  ...
  setSection(view, bits, shoff, 4, {
    type: bits === 64 ? SHT_RELA : SHT_REL,
    offset: relocOffset,
    size: relocEnt + remainder,
    link: 2,
    info: 3,
    align: bits === 64 ? 8 : 4,
    entsize: relocEnt,
...
function buildDynamicELF(bits, remainder) {
  const strOffset = 0x80;
  const dynamicOffset = 0x100;
  const shoff = 0x300;
  const shnum = 3;
  const shsize = bits === 64 ? 64 : 40;
  const bytes = new Uint8Array(shoff + shnum * shsize);
  const view = new DataView(bytes.buffer);
  setHeader(view, bits, { type: ET_DYN, shoff, shnum });
  bytes[strOffset] = 0;
  const ent = bits === 64 ? 16 : 8;
  if (bits === 64) {
    view.setBigInt64(dynamicOffset, 0n, true); // DT_NULL
    view.setBigUint64(dynamicOffset + 8, 0n, true);
  } else {
    view.setInt32(dynamicOffset, 0, true);
    view.setUint32(dynamicOffset + 4, 0, true);
  }
