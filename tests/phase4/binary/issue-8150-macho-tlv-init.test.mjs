...
test('unknown Mach-O ISA cannot mint an exact TLV initializer start', () => {
  const bytes = build64({ pointers: [0x1180n] });
  new DataView(bytes.buffer).setInt32(4, 0x12345678, true);
  const image = parseMachO(bytes);
  assert.equal(image.functions.length, 0);
  assert.equal(image.metadata.tlvInitializers[0].valid, false);
  assert.ok(image.metadata.machoMetadata.reasons.some((r) => r.includes('tlv-init:unsupported-isa')));
});
