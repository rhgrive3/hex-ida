...
test('dyld slide-info v5 page_starts entries are pointer units', async () => {
  const mapping = {
  ...
  assert.deepEqual(reads, [mapping.fileOffset + 0x58n * 8n]);
  assert.equal(records.length, 1);
  assert.equal(records[0].storageAddress, mapping.address + 0x58n * 8n);
