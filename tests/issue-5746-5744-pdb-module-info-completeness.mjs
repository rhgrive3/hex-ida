...
// 3b. Valid names whose required 4-byte alignment crosses the declared end
// must be rejected before the entry is published.
{
  const bytes = new Uint8Array(DBI_HEADER_SIZE + 67);
  const view = new DataView(bytes.buffer);
  view.setInt16(DBI_HEADER_SIZE + 34, 5, true);
  view.setUint32(DBI_HEADER_SIZE + 36, 0, true);
  bytes.set(new TextEncoder().encode('m\\0\\0'), DBI_HEADER_SIZE + 64);
  const result = parseModuleInfo(bytes, { moduleSubstreamSize: 67 });
  assert.equal(result.complete, false, 'aligned entry boundary past the declared end must be incomplete');
  assert.deepEqual(result.modules, [], 'an entry crossing the declared end must not be published');
}
...
// 4b. A truly empty module list (no substream declared) is complete-and-empty.
{
...
// 6. Nullish inputs stay complete-and-empty (nothing declared).
{
  assert.deepEqual(parseModuleInfo(null, { moduleSubstreamSize: 128 }), { modules: [], complete: true });
  assert.deepEqual(parseModuleInfo(new Uint8Array(128), { moduleSubstreamSize: 0 }), { modules: [], complete: true });
}
