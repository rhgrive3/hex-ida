...
function structuralClaim(descriptor) {
  return { layer: 'structural', entityId: 'issue-4427-entity', descriptor };
}
function createNestedClaim(nested) {
  return createTypeClaim(structuralClaim({ offset: 0, sizeBytes: 32, memberType: nested }));
}
...
test('#4427 rejects invalid numeric facts at every nested type depth', () => {
  const cases = [
    ...
    [{ kind: 'struct', members: [{ offset: -1, sizeBytes: 4, memberType: integer32 }] }, 'structural-offset-invalid'],
    [{ kind: 'struct', members: [{ offset: 0, sizeBytes: 4, memberType: { kind: 'array', strideBytes: 4, length: 2, elementType: { kind: 'struct', members: [{ offset: 0, sizeBytes: -1 }] } } }] }, 'structural-size-invalid'],
  ];
  for (const [nested, error] of cases) assert.throws(() => createNestedClaim(nested), new RegExp(error));
});
...
test('#4427 rejects invalid nested hard evidence before it can become certain', () => {
  ...
  assert.throws(() => graph.addHardConstraint({
    ...
    claim: structuralClaim({ offset: 0, sizeBytes: 8, memberType: { kind: 'array', strideBytes: 8, length: 2, elementType: { kind: 'integer', widthBits: 32, length: -1 } } }),
...
test('#4427 preserves valid recursive nested descriptors and numeric spellings', () => {
  ...
  graph.addHardConstraint({ kind: 'debug-type', origin: 'debug-matched', claim: structuralClaim({
    offset: '0', sizeBytes: '32', alignBytes: '8', memberType: { kind: 'struct', members: [{
      offset: '0', sizeBytes: '16', alignBytes: '8', memberType: { kind: 'array', strideBytes: '8', length: '2', elementType: { kind: 'pointer', targetEntityId: 'issue-4427-entity', pointeeType: { kind: 'integer', widthBits: '64' } } },
    }] },
  }) });
  ...
  assert.equal(result.layers.structural.selected.descriptor.members[0].memberType.members[0].memberType.length, '2');
  assert.equal(result.layers.structural.selected.descriptor.members[0].memberType.members[0].memberType.elementType.targetEntityId, 'issue-4427-entity');
});
