...
test('#6066 lookalike objects with proof-shaped metadata stay unproven', () => {
  ...
  assert.equal(aliasOf(singleton(lookalike), singleton(createPointsToTarget({
    addressSpace: 'memory', rootKind: 'rooted', rootEntityId: 'b', offsetRange: exactRange(0),
  }))).relation, 'may');
});
...
test('#6066 internal reconstructions of proven targets keep their authority', () => {
  ...
  }, { separationClass: 'tls-like', separationAuthority: 'root-descriptor' });
