...
test('per-call resolver terminates only the direct call bound to noreturn evidence', () => {
  const fatalTarget = 0x1000n;
  const normalTarget = 0x2000n;
  const seen = [];
  const blocks = partitionDecodedFunction(decoded([
    [0, 'call', fatalTarget],
    [4, 'fallthrough'],
    [8, 'call', normalTarget],
    [12, 'return'],
...
test('unknown per-call prototype keeps ordinary call fallthrough', () => {
  const blocks = partitionDecodedFunction(decoded([
    [0, 'call', 0x1000n],
    [4, 'return'],
...
test('indirect calls can bind noreturn evidence by callsite identity', () => {
  const blocks = partitionDecodedFunction(decoded([
    [0, 'call'],
    [4, 'return'],
  ]), plugin(), {
...
test('instruction-bound evidence outranks the resolver for the same callsite', () => {
  let resolverCalls = 0;
  const blocks = partitionDecodedFunction(decoded([
...
test('legacy global noreturn fallback is retained only for a single callsite', () => {
  const single = partitionDecodedFunction(decoded([
    [0, 'call', 0x1000n],
    [4, 'return'],
  ]), plugin(), { callPrototype:{ noreturn:true } });
  assert.equal(single.length, 2, 'PR #944 single-call behavior must remain');

  const multiple = partitionDecodedFunction(decoded([
    [0, 'call', 0x1000n],
    [4, 'fallthrough'],
    [8, 'call', 0x2000n],
    [12, 'return'],
...
test('resolver is evaluated once per callsite so CFG passes consume one stable authority', () => {
  let calls = 0;
  const blocks = partitionDecodedFunction(decoded([
    [0, 'call', 0x1000n],
    [4, 'return'],
