...
function signedInt(offset = 0x20, name = 'int32') {
  return die(offset, TAG.base, [
    [AT.name, attr(name)],
    [AT.byteSize, attr(4n, 0x0b)],
    [AT.encoding, attr(0x05n, 0x0b)],
  ]);
}
...
function typeRecord({ pointer, targets = [], wrapper = null, variableOffset = 1 }) {
  ...
  const records = new DwarfDebugInfoProvider().types(resultFor(dies), {}).records;
...
test('#4657 a resolved specification may supply the pointer target', () => {
  ...
  const record = typeRecord({ pointer, targets: [declaration, signedInt()] });
  assert.equal(record.descriptor.claim.name, 'int32 *');
  assert.equal(record.descriptor.complete, true);
});

test('#4657 a self-referential pointer target does not become complete through a cycle', () => {
  const pointer = die(0x10, TAG.pointer, [[AT.type, typeRef(0x10)]]);
  const record = typeRecord({ pointer });
  assert.equal(record.descriptor.claim.name, 'unknown *');
  assert.equal(record.descriptor.complete, false);
});
...
test('#4657 a direct pointee cannot bypass a broken pointer specification chain', () => {
  const pointer = die(0x10, TAG.pointer, [
    [AT.type, typeRef(0x20)],
    [AT.specification, typeRef(0x77)],
  ]);
  const record = typeRecord({ pointer, targets: [signedInt()] });
  assert.equal(record.descriptor.claim.name, 'int32 *');
  assert.equal(record.descriptor.complete, false);
