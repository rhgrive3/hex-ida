...
function record(input = {}) {
  return createDebugRecord({
    ...
    providerId: 'pdb',
    providerVersion: '1',
    buildIdentity: input.buildIdentity ?? 'build-A',
...
test('matched-partial coverage outside the record fails closed to soft evidence', () => {
  ...
  assert.deepEqual(applyDebugTypesToGraph(graph, result, createDebugPage({ records: [outside] })), { hard: 0, soft: 1, skipped: 0 });
...
test('matched-partial record explicitly covered by entity id may be authoritative', () => {
  ...
  assert.deepEqual(applyDebugTypesToGraph(graph, result, createDebugPage({ records: [covered] })), { hard: 1, soft: 0, skipped: 0 });
