...
test('#5130 proven separations outside the entry-argument rule are preserved', () => {
  assert.equal(aliasMemoryRegions(stackRegion({ offset: '-16' }), stackRegion({ offset: '32' })), 'no');
  assert.equal(aliasMemoryRegions(stackRegion({ offset: '-16' }), stackRegion({ offset: '-12' })), 'may');
  assert.equal(
    aliasMemoryRegions(
      stackRegion(),
