...
test('issue 6222: ensurePage relays abort if signal becomes aborted during listener registration', async () => {
  let innerControllerSignal = null;
  const disassembleAt = async (_addr, { signal }) => {
  ...
  const index = new VariableInstructionIndex({ disassembleAt, pageBytes: 64 });
  ...
  await assert.rejects(() => index.ensurePage(0x1000n, { signal }), /AbortError|aborted/);
