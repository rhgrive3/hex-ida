...
test('6036: uintptr_t/intptr_t/ptrdiff_t are integers in parameterClass', () => {
  for (const type of ['uintptr_t', 'intptr_t', 'ptrdiff_t', 'pointer_sized_int']) {
...
test('6036: sysv/aapcs64/darwin/riscv agree on integer typedefs', () => {
  ...
  const darwinPointerSized = classifyDarwinArm64Arguments({ callPrototype: { args: [{ type: 'pointer_sized_int' }] } });
  assert.equal(darwinPointerSized.arguments[0].pointer ?? false, false);
