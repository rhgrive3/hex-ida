...
}
{
  ...
  assert.throws(() => image.readVirtual(0n, BigInt(Number.MAX_SAFE_INTEGER)), (error) => error?.code === 'BINARY_VIRTUAL_READ_RESOURCE_LIMIT' && error.requested === BigInt(Number.MAX_SAFE_INTEGER) && error.limit > 0n && error.limit < error.requested);
}
{
