...
function irFor(blocks) {
  const nodes = blocks.flatMap(({ id: blockId, nodeIds }) => nodeIds.map((id) => ({
    // A return node is a valid zero-arity fixture. `copy` has a strict
    // one-input/one-output contract and would fail before these tests reach
    // the source-node membership checks they are intended to exercise.
...
test('#4534 ordinary definitions require an existing source node in their definition block', () => {
  ...
  assert.throws(
    () => validateSemanticSsa(
      ...
      irFor(twoBlocks), cfgFor(twoBlocks),
    ),
