...
test('#6154 scope-only claims fail closed without matching evidence scope', () => {
  const scope = { kind:'function', functionId:'entity-B' };
  const graph = new EvidenceGraph({
    nodes: [
      {
        id:'proof-for-A',
  ...
  assert.notEqual(graph.evaluateClaim('claim-for-B-by-scope').verdict, 'confirmed');
  assert.equal(
    canConfirmClaim(
      { deterministic:true, completeness:'complete', targetEntityIds:['entity-A'], binaryId:null },
      { targetEntityIds:[], scope, binaryId:null },
    ),
    false,
  );
  assert.equal(
    canConfirmClaim(
      { deterministic:true, completeness:'complete', payload:{ scope }, binaryId:null },
      { targetEntityIds:[], scope, binaryId:null },
    ),
    true,
  );
