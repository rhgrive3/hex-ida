...
function aapcs64(type) {
  return classifyAAPCS64Arguments({ callPrototype:{ args:[{ type, ...AGGREGATE }] } }).arguments[0];
}

function darwin(type) {
  return classifyDarwinArm64Arguments({ callPrototype:{ args:[{ type, ...AGGREGATE }] } }).arguments[0];
}
...
test('#6012: explicit pointer metadata keeps priority over type text', () => {
  for (const classify of [aapcs64, darwin]) {
    ...
    const flagged = classifyAAPCS64Arguments({ callPrototype:{ args:[
      { type:'value_t', pointer:true, ...AGGREGATE },
    ] } }).arguments[0];
...
test('#6012: large aggregates keep the indirect path and >16-byte layout rules', () => {
  ...
  const sysv = classifySysVAMD64Arguments({ callPrototype:{ args:[{ type:'struct Big', aggregate:true, bits:192, bytes:24 }] } }).arguments[0];
  assert.ok(sysv, 'sysv classification is unchanged for aggregates');
  const win = classifyMicrosoftX64Arguments({ callPrototype:{ args:[{ type:'struct Big', aggregate:true, bits:192, bytes:24 }] } }).arguments[0];
  assert.ok(win, 'microsoft-x64 classification is unchanged for aggregates');
});
