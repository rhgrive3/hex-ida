...
test('#5603: long double pointers stay in the pointer/integer lane', () => {
  const arg = classifyDarwinArm64Arguments({
    callPrototype:{ args:[{ type:'long double *' }] },
  }).arguments[0];
  assert.equal(arg.reg, 'x0');
  assert.equal(arg.pointer, true);
  assert.notEqual(arg.abiClass, 'fp');

  const fnReturn = DARWIN_ARM64_ABI.classifyFunctionReturn({
    functionPrototype:{ returnType:'long double *', returnsValue:true },
  });
  assert.equal(fnReturn.reg, 'x0');
  assert.notEqual(fnReturn.reg, 'v0');

  const callReturn = DARWIN_ARM64_ABI.classifyCallReturn({
    callPrototype:{ returnType:'long double *', returnsValue:true },
  });
  assert.equal(callReturn.reg, 'x0');
  assert.notEqual(callReturn.reg, 'v0');

  const widthAnnotated = DARWIN_ARM64_ABI.classifyFunctionReturn({
    functionPrototype:{ returnType:'long double *', returnBits:128, returnsValue:true },
  });
  assert.equal(widthAnnotated.reg, 'x0');
  assert.notEqual(widthAnnotated.partial, true);
});
