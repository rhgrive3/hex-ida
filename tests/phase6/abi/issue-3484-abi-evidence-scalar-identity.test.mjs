...
function canonicalEvidence({ binaryId = 'binary-X', sliceId = 'slice-1' } = {}) {
...
assert.equal(typeof canonical.abiIdentity.schemaVersion, 'number');

{
  const nullish = canonicalEvidence({ sliceId:null });
  nullish.provenance.sliceId = undefined;
  assert.equal(
    canonicalAbiEvidence(nullish),
    true,
    'null and undefined must retain the existing equivalent optional-field contract',
  );
}
