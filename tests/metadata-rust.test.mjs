...
// 7. RustMetadataProvider probe and symbol generation
{
  const provider = new RustMetadataProvider({
    symbols: [
      ...
      { symbol: '_ZN6my_app13MyTraitvtable17h1122334455667788E', addr: '0x3000', size: 16, isVtable: true },
    ],
...
// 9. Equivalent numeric/decimal/hex addresses share one canonical identity.
{
  ...
  const numeric = new RustMetadataProvider({
    symbols: [
      ...
      { name: vtableName, address: 8192n, isVtable: true },
    ],
  ...
  const textual = new RustMetadataProvider({
    symbols: [
      ...
      { name: vtableName, address: '8192', isVtable: true },
    ],
...
await import('./phase12/integration/issue-5864-rust-nested-identifier.test.mjs');
