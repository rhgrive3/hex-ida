...
// Settle runtime cases before the canonical import runner advances to modules
// that temporarily instrument global timers.
await test('#4456 structured store selectors cannot alias a canonical session', async () => {
  let loadCalls = 0;
  let deleteCalls = 0;
  const persistence = {
...
await test('#4456 explicit session IDs require a non-empty primitive string', () => {
...
await test('#4456 request and runtime boundaries reject structured session IDs', async () => {
...
await test('#4456 persistence selectors preserve exact string identity', async () => {
...
await test('#4456 valid session strings still persist, reload, update, and release', async () => {
