...
assert.throws(() => transport({ endpoint:['https://collaboration.example.test/authorize'] }), /remote-transport-endpoint-required/);
assert.throws(() => transport({ serverKeyId:['server:key:1'] }), /remote-transport-server-key-id-required/);
...
for (const [field, code] of [
  ...
  const current = transport({ fetchImpl:signedFetch((response) => ({ ...response, [field]:[response[field]] })) });
...
{
  const current = transport({ fetchImpl:signedFetch((response) => ({ ...response, signature:[response.signature] })) });
  await assert.rejects(
    () => current.authorizeEnvelope(input(sequence++)),
...
console.log('[phase12] issue-8539 remote transport identity boundary tests passed');
