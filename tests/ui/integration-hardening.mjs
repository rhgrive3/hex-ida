...
const productBase = fs.readFileSync(path.join(ROOT, 'js/ui/product-base.js'), 'utf8');
...
assert.equal(productSource.includes('triggerLegacyInvestigation'), false, 'canonical investigate must not fake legacy sheet input');
assert.equal(productSource.includes('HP・体力') && productSource.includes('ダメージ計算') && productSource.includes('所持金・コイン'), true, 'canonical investigate must expose beginner goal presets');
assert.equal(productSource.includes('renderSecondaryRoute(app, router, route, routeContext)'), true, 'settings/help/learn must render as canonical routes under the router-owned task scope');
assert.equal(productSource.includes('function renderSecondary('), false, 'legacy open-button secondary screen must be removed');
assert.equal(productSource.includes('showSettings, showHelp, showLearn'), false, 'product UI must not import legacy secondary sheets');
...
assert.equal(productSource.includes('querySelector(\'#overlays .sheet'), false, 'canonical investigate must not scrape legacy sheet DOM');
assert.equal(productSource.includes('compileGoal('), true, 'canonical investigate must call Goal Compiler directly');
assert.equal(productSource.includes('traceAppFunction('), true, 'Function Runtime tab must call Runtime Analysis Platform bridge');
assert.equal(productSource.includes('runtimeEvidenceForApp('), true, 'Function Evidence tab must surface runtime evidence');
assert.equal(productSource.includes('classifyFunction('), true, 'Function Overview must surface recognition classification');
assert.equal(productSource.includes('out.length < 1000'), false, 'Explorer must not retain the old 1000 function cap');
assert.equal(productSource.includes('functionList(app.codeRegion(), 600)'), false, 'Explorer must not retain the old 600 function cap');
