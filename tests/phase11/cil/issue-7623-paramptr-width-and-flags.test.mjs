...
test('#7623 Param Sequence gaps are permitted when every row stays within the method arity', () => {
  const rows = new Uint8Array(12);
  const view = new DataView(rows.buffer);
  view.setUint16(0, 0, true); view.setUint16(2, 1, true);
  view.setUint16(6, 0, true); view.setUint16(8, 3, true);
  const { bytes } = buildCil({
    methods: [{ name:'Run', body:[0x2a], signature:[0, 3, 1, 0x08, 0x08, 0x08] }],
    extraRows:[[0x08, { count:2, bytes:rows }]],
  });
  const image = parseCil(bytes);
  assert.deepEqual(image.params.map((row) => row.sequence), [1, 3]);
});
