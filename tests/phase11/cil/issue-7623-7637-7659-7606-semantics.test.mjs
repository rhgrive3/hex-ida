...
function parseWithStrings(caseOptions) {
  ...
  const bytes = buildCil({
    ...
    streams: [
      ...
      { name: '#Blob', bytes: pad(Uint8Array.of(0, 3, 0, 0, 1, 2, 6, 8)) },
    ],
  }).bytes;
  return bytes;
}
...
test('#7623 differing In/Out flags change the canonical method surface', () => {
  const paramRow = (flags) => {
    ...
    return parseWithStrings({ extraRows: [[0x08, { count: 1, bytes: params }]] });
  };
...
test('#7637 PropertyMap/Property/MethodSemantics reach the canonical type surface', () => {
  ...
  prv.setUint16(0, 0, true); prv.setUint16(2, 22, true); prv.setUint16(4, 1, true); // 'Value', blob 1
  ...
  msv.setUint16(2, 2, true); // MethodDefOrRef coded: MethodDef rid 1
...
test('#7659 EventMap/Event/MethodSemantics reach the canonical type surface', () => {
  ...
  msv.setUint16(2, 2, true); // MethodDef rid 1
