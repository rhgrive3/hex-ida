...

{
  ...
  assert.throws(
    ...
    /cil-metadata-version-truncated|cil-unsupported-binary/,
  );
  assert.throws(
    () => parseCilBase(bytes.slice(0, versionEnd - 1)),
...

{
  ...
  assert.throws(
    ...
    /cil-metadata-stream-header-truncated|cil-unsupported-binary/,
  );
  assert.throws(
    () => parseCilBase(bytes.slice(0, flagsOffset + 2)),
...

{
  ...
  assert.throws(
    () => parseCilBase(bytes.slice(0, streamHeader + 7)),
    /cil-metadata-stream-header-truncated/,
  );
}

{
  ...
  assert.throws(
    () => parseCil(bytes),
    /cil-metadata-stream-out-of-bounds|cil-unsupported-binary/,
  );
  assert.throws(
    () => parseCilBase(bytes),
