...
function parseWithBlob(caseOptions, blobs, fail) {
  // Keep the fixture builder's canonical legacy #Blob prefix (including the
  // default FIELD I4 signature at index 5), append Constant payloads through
  // options.blobs, and retarget Constant.Value indices from the old compact
  // payload-only heap to those appended blobs. This keeps #~ and #Blob from a
  // single coherent build so FieldSig authority remains provable.
  const compact = blobHeap(blobs);
  let nextIndex = 8; // medium-cil's legacy #Blob prefix length
  const appendedIndexes = blobs.map((value) => {
    const index = nextIndex;
    nextIndex += 1 + value.length;
    return index;
  });
  const remap = new Map(compact.indexes.map((index, i) => [index, appendedIndexes[i]]));
  const extraRows = (caseOptions.extraRows ?? []).map(([table, row]) => {
    if (table !== 0x0b) return [table, row];
    const bytes = row.bytes.slice();
    const view = new DataView(bytes.buffer, bytes.byteOffset, bytes.byteLength);
    for (let i = 0; i < row.count; i++) {
      const current = view.getUint16(i * 6 + 4, true);
      if (remap.has(current)) view.setUint16(i * 6 + 4, remap.get(current), true);
    }
    return [table, { ...row, bytes }];
  });
  const bytes = buildCil({
    ...caseOptions,
    blobs: blobs.map((value) => Uint8Array.from(value)),
    extraRows,
  }).bytes;
  if (fail) {
  ...
  return { image: parseCil(bytes), blob: { indexes: appendedIndexes }, bytes };
}
...
test('#7552 constant value shapes decode exactly', () => {
  ...
  const { image, blob } = parseWithBlob(
    { fields: values.map(([type], i) => ({ name: `F${i}`, flags: 0x8056, signature: type === 0x12 ? [0x06, 0x12, 0x04] : [0x06, type] })), extraRows: [[0x0b, { count: values.length, bytes: rows }]] },
...
test('#7552 64-bit Constant high-bit boundaries decode exactly', () => {
  ...
  const { image } = parseWithBlob(
    { fields: values.map(([type], i) => ({ name: `B${i}`, flags: 0x8056, signature: [0x06, type] })), extraRows: [[0x0b, { count: values.length, bytes: rows }]] },
    values.map(([, payload]) => payload),
  );
...
test('#7552 Constant.Type OBJECT (0x1c) is not a valid null-reference encoding', () => {
  ...
  const ok = parseWithBlob({ fields: [{ name: 'X', flags: 0x8056, signature: [0x06, 0x12, 0x04] }], extraRows: [[0x0b, { count: 1, bytes: classRow }]] }, zeroBlob);
