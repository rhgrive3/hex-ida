...
for (const [label, bytes] of [
  ...
  assert.equal(parsed.elements[0].refType, FUNCREF, `${label}: legacy index segment carries funcref authority`);
}
...
for (const [label, bytes] of [
  ...
  assert.throws(
    () => parseWasm(bytes),
    (error) => error instanceof TypeError && error.message === 'wasm-invalid-element-table-type-mismatch',
