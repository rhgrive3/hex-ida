...
for (const field of ['binaryId', 'format', 'architecture', 'loaderVersion', 'sourceHash']) {
  for (const value of [null, undefined]) {
    malformed.push([`optional ${field} ${value}`, validStrings({[field]:value}), 'rebuild-v2-publication-identity-invalid']);
  }
  assert.equal((await publish(validStrings({[field]:materialized[field]}))).status, 'published', `${field}: valid optional identity stays accepted`);
  const omitted = validStrings({[field]:null});
  delete omitted[field];
  assert.equal((await publish(omitted)).status, 'published', `${field}: genuine omission stays accepted`);
}
