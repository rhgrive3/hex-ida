...
// #6003: stack offsets follow the parameter position so register-argument
// shadow slots at positions 4/5 are never collapsed into later stack slots.
{
  const sixVectors = Array.from({ length:6 }, () => ({ type:'__m128', vector:true, bits:128 }));
  const result = classifyMicrosoftVectorcallArguments({ callPrototype:{
    callingConvention:'__vectorcall',
    args:[...sixVectors, { type:'int', bits:32 }],
  } });
  assert.deepEqual(result.arguments.slice(0, 6).map((argument) => argument.reg), ['xmm0','xmm1','xmm2','xmm3','xmm4','xmm5']);
  const seventh = result.arguments[6];
  assert.equal(seventh.location, 'stack');
  assert.equal(seventh.offset, 48, 'position 6 must land past the position 4/5 shadow slots');
  assert.equal(seventh.calleeEntryOffset, 56);
  assert.equal(seventh.offsetBase, 'caller-stack-before-call');
}

// A stack argument at position 4 sits at +32, a register vector at position 5
// consumes its own shadow slot, and the next stack argument is still +48.
// (The vector register's position mapping itself is tracked by #5586.)
{
  const result = classifyMicrosoftVectorcallArguments({ callPrototype:{
    callingConvention:'__vectorcall',
    args:[
      { type:'int', bits:32 }, { type:'int', bits:32 }, { type:'int', bits:32 },
      { type:'int', bits:32 }, { type:'int', bits:32 }, { type:'__m128', vector:true, bits:128 },
      { type:'int', bits:32 },
    ],
  } });
  const positionFour = result.arguments[4];
  assert.equal(positionFour.location, 'stack');
  assert.equal(positionFour.offset, 32);
  assert.equal(result.arguments[5].location, 'register', 'position 5 vector must stay in a vector register');
  const positionSix = result.arguments[6];
  assert.equal(positionSix.location, 'stack');
  assert.equal(positionSix.offset, 48, 'position 5 shadow slot must reserve +40');
}

// All-stack tails keep consecutive positional slots: two stack arguments at
// positions 4 and 5 land at +32/+40.
{
  const result = classifyMicrosoftVectorcallArguments({ callPrototype:{
    callingConvention:'__vectorcall',
    args:Array.from({ length:6 }, () => ({ type:'uint64_t', bits:64 })),
  } });
  assert.equal(result.arguments[5].offset, 40);
  assert.equal(result.arguments[4].offset, 32);
}

// Standard integer-only layout keeps the historical first-four offsets.
{
  const result = classifyMicrosoftVectorcallArguments({ callPrototype:{
    callingConvention:'__vectorcall',
    args:[
      { type:'int', bits:32 }, { type:'int', bits:32 }, { type:'int', bits:32 },
      { type:'int', bits:32 }, { type:'uint64_t', bits:64 }, { type:'uint64_t', bits:64 },
      { type:'uint64_t', bits:64 },
    ],
  } });
  assert.deepEqual(result.arguments.slice(0, 4).map((argument) => argument.reg), ['rcx','rdx','r8','r9']);
  assert.deepEqual(result.arguments.slice(4).map((argument) => argument.offset), [32, 40, 48]);
}

console.log('issues #526/#954/#955/#958/#959/#6003 ABI regressions: PASS');
