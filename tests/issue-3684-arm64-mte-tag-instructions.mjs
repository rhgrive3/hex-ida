...
try {
  ...
  assert.equal(ldg.pseudo, 'x0 = WithAllocationTag(x0, AllocationTag(x1))',
    'LDG must merge the loaded tag into the existing destination pointer');
  assert.match(rendered(ldg), /existing .*value|preserv.*non-tag address bits|merge/i,
    'LDG must explain destination-address preservation');
  assert.doesNotMatch(rendered(ldg), /data (?:address )?bits (?:read as )?zero/i,
    'LDG must not claim that destination address bits are zeroed');
