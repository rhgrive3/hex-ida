...
const candidateTreeSha = '3'.repeat(40);
assert.match(candidateTreeSha, /^[0-9a-f]{40}$/);
const assignedBase = spawnSync('git', ['rev-parse', '60980a3c9312b1dda7619d5e88b4a97df1016276'], { encoding: 'utf8' }).stdout.trim();
...
const report = createOracleReport({
  ...
  toolchain: {
    ...
    status: 'not-verified',
    diagnostics: ['unit fixture only; external tool execution not verified'],
  },
...
assert.throws(() => createOracleReport({
  ...
  externalEvidence: [{
    ...
    authorityId: 'unregistered-authority',
  }],
}), /report-external-evidence-authority-mismatch/);
assert.throws(() => createOracleReport({
  productSha: currentHead,
  baseSha: assignedBase,
  corpus,
  results,
  externalEvidence: [report.externalEvidence[0], report.externalEvidence[0]],
}), /report-external-evidence-profile-duplicate/);
...
const missingResultReport = createOracleReport({
  productSha: currentHead,
  baseSha: assignedBase,
  corpus,
...
assert.equal(missingResultReport.totalCount, corpus.cases.length);
assert.equal(missingResultReport.counts['not-integrated'], 1);
assert.equal(missingResultReport.blockingCount, 1);
assert.ok(missingResultReport.nonPassReasons.some((item) => item.code === 'result-missing'));
assert.throws(() => assertReleaseReady(missingResultReport), /report-blocking-results/);
