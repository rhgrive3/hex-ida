...
await forEachX86BrowserSession(async ({ decodeAndLift }) => {
  for (const [raw, mnemonic] of [
    ...
    const [{ decoded, effects }] = await decodeAndLift(raw, 0x1000n);
  ...
  for (const [raw, family] of [
    ...
    const [{ decoded, effects }] = await decodeAndLift(raw, 0x2000n);
  ...
  const [{ decoded:fcomi, effects:fcomiEffects }] = await decodeAndLift([0xdb, 0xf1], 0x2100n);
  ...
  const [{ decoded:add, effects:addEffects }] = await decodeAndLift([0x01, 0xd8], 0x3000n);
