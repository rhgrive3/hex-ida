...
try{
  ...
  assert.throws(()=>decoded({family:'mov',operands:[reg('rax',64),mem({base:'rax',widthBits:64,addressSizeBits:16})],addressSizeBits:16}),/x86-decoded-instruction-invalid-address-size/);
  for(const malformed of [decoded({family:'mov',operands:[reg('rax',64),mem({base:'rax',widthBits:24})]}),decoded({family:'mov',operands:[mem({base:'rax',widthBits:64}),mem({base:'rbx',widthBits:64})]})]){const effects=liftX86MemoryEffects(malformed);assert.equal(effects.completeness,'partial');assert.equal(effects.metadata.failClosed,true);}
  assert.throws(()=>decoded({family:'mov',operands:[reg('rax',64),mem({base:'rax',widthBits:64,segment:'bogus'})]}),/x86-decoded-instruction-invalid-memory-segment/);
