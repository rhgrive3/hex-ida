...
try{
  ...
  for(const malformed of [decoded({family:'mov',operands:[reg('rax',64),mem({base:'rax',widthBits:24})]}),decoded({family:'mov',operands:[reg('rax',64),mem({base:'rax',widthBits:64,addressSizeBits:16})],addressSizeBits:16}),decoded({family:'mov',operands:[mem({base:'rax',widthBits:64}),mem({base:'rbx',widthBits:64})]})]){const effects=liftX86MemoryEffects(malformed);assert.equal(effects.completeness,'partial');assert.equal(effects.metadata.failClosed,true);}
