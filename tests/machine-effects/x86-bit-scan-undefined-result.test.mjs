...
test('raw BSF/BSR bytes must bind every decoded register and memory-address field', async () => {
  ...
  try {
    ...
    const rexXNoIndex = one(session, [0x44,0x0f,0xbc,0x44,0x25,0x00]);
    const forged = [
    ...
    for (const [id, decoded] of [['rex-extended', extended], ['sib-displacement', sibDisp], ['rex-x-no-index', rexXNoIndex]]) {
      ...
      if (id === 'rex-x-no-index') {
        assert.equal(decoded.detail.operands[1].memory.index, null, id);
        assert.equal(decoded.detail.operands[1].memory.base, 'rbp', id);
        assert.equal(decoded.detail.operands[1].memory.displacement, 0n, id);
      }
    }
  } finally {
    session.close();
