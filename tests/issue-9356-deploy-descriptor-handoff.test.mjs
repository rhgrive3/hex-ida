...
test('#9356 pathname swap during validator cannot change the descriptor-bound input', () => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), 'hex-9356-validator-race-'));
  try {
    ...
    const status = runProductionDeploy({
      ...
      run(_file, args, options) {
        calls += 1;
        if (calls === 1) {
          assert.equal(args[1], '--config=/proc/self/fd/3');
          ...
          fs.writeFileSync(snapshotPath, approved);
          return ok;
        }
        assert.deepEqual(args.slice(-2), ['--config', '/proc/self/fd/3']);
        assert.equal(options.stdio[3], 3);
