...
const identities = [
  [['format'], 'format', (t) => t.format, 'validator-format-invalid'],
  [['architecture', 'arch'], 'arch', (t) => t.architecture, 'validator-architecture-invalid'],
  [['loaderVersion', 'parserVersion'], 'loaderVersion', (t) => t.loaderVersion, 'validator-loader-identity-invalid'],
  [['sourceHash', 'inputHash'], 'sourceHash', (t) => t.sourceHash, 'validator-source-identity-invalid'],
  [['outputHash', 'bytesHash'], 'outputHash', (_t, m) => m.outputHash, 'validator-output-identity-invalid'],
];
for (const [fields, imageField, expected, reason] of identities) {
  for (const value of [null, undefined]) {
    for (const field of fields) {
      structured.push([`${field} ${value}`, () => ({ ok:true, [field]:value }), reason]);
      structured.push([`${field} ${value} with valid image fallback`, (_ctx, t, m) => ({
        ok:true, [field]:value, image:{[imageField]:expected(t, m)},
      }), reason]);
    }
    structured.push([`image.${imageField} ${value}`, () => ({ ok:true, image:{[imageField]:value} }), reason]);
    structured.push([`image.${imageField} ${value} with valid primary identity`, (_ctx, t, m) => ({
      ok:true, [fields[0]]:expected(t, m), image:{[imageField]:value},
    }), reason]);
    if (fields.length > 1) {
      structured.push([`${fields[1]} ${value} with valid primary identity`, (_ctx, t, m) => ({
        ok:true, [fields[0]]:expected(t, m), [fields[1]]:value,
      }), reason]);
      structured.push([`${fields[0]} ${value} with valid alias`, (_ctx, t, m) => ({
        ok:true, [fields[0]]:value, [fields[1]]:expected(t, m),
      }), reason]);
    }
  }
}
...
for (const [fields, imageField, expected, reason] of identities) {
  for (const field of fields) {
    ...
    assert.equal(validation.status, 'valid', `${field}: valid identity must pass`);
  }
  const { validation } = await runValidator((_ctx, t, m) => ({ ok:true, image:{[imageField]:expected(t, m)} }));
