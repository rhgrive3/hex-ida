...
test('real native owners propose an unchecked conclusion; only replay derives the scoped integer fact', async t => {
  ...
  assert.deepEqual(proofs.map(row => row.payload.fragment.conclusion.constant).sort(), ['8176', '8192']);
  assert.ok(proofs.every(row => row.payload.proofStatus === 'not-checked'));
  assert.equal(new Set(proofs.map(row => row.payload.fragment.semanticValueId)).size, 2);
  const ranges = before.graph.nodes.filter(n => n.semanticKind === RANGE_PROOF_KIND);
  assert.equal(ranges.length, proofs.length);
  for (const row of ranges) {
    const rule = row.payload.rule, primitive = proofs.find(p => rule.premises.length === 1 && rule.premises[0] === p.id);
    assert.ok(primitive); assert.equal(rule.schema, RANGE_PROOF_SCHEMA); assert.equal(rule.rule, 'integer-range-projection');
    assert.equal(row.payload.proofStatus, 'not-checked');
    assert.deepEqual(rule.scope, integerFragmentProofScope(primitive.payload.fragment));
    assert.deepEqual(rule.conclusion, { kind: 'unsigned-range', subject: primitive.payload.fragment.semanticValueId,
      bits: primitive.payload.fragment.conclusion.bits, lower: primitive.payload.fragment.conclusion.range.lower,
      upper: primitive.payload.fragment.conclusion.range.upper });
  }
  ...
  assert.equal(replay.derivation.status, 'partially-checked');
  assert.deepEqual([...replay.derivation.checkedNodeIds].sort(), [...proofs, ...ranges].map(row => row.id).sort());
  for (const node of ranges) {
    const checked = replay.nodeResults.find(row => row.nodeId === node.id);
    assert.equal(checked.status, 'verified'); assert.equal(checked.detail.premisesChecked, 1);
    assert.deepEqual(checked.detail.scope, node.payload.rule.scope);
    assert.deepEqual(checked.detail.proposition, node.payload.rule.conclusion);
  }
  assert.equal(replay.ownerReplay.counters.integerDerivations, 2); assert.equal(replay.ownerReplay.counters.integerRejected, 0);
...
test('integrity-only replay never inherits prior successful derivation authority', async t => {
  ...
  assert.deepEqual(owners.nodeResults.filter(row => owners.derivation.checkedNodeIds.includes(row.nodeId)
    && row.detail.constant != null).map(row => row.detail.constant).sort(), ['8176', '8192']);
  assert.deepEqual(owners.nodeResults.filter(row => owners.derivation.checkedNodeIds.includes(row.nodeId)
    && row.detail.rule === 'integer-range-projection').map(row => row.detail.proposition.lower).sort(), ['8176', '8192']);
