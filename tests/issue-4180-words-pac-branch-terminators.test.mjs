...
const UNALLOCATED_BRAAZ_RM1 = 0xd61f0801;
const UNALLOCATED_BRABZ_RM1 = 0xd61f0c01;
const UNALLOCATED_RETAA_RN0 = 0xd65f081f;
const UNALLOCATED_RETAB_RN0 = 0xd65f0c1f;
...
test('#4180 authenticated terminators keep their architecturally fixed register fields', () => {
  const invalidBranches = [UNALLOCATED_BRAAZ_RM1, UNALLOCATED_BRABZ_RM1];
  const invalidReturns = [UNALLOCATED_RETAA_RN0, UNALLOCATED_RETAB_RN0];
  for (const word of invalidBranches) {
    assert.equal(W.isBr(word), false, `0x${word.toString(16)} has Z=0 with Rm != 31`);
    assert.notEqual(W.classifyWord(word), K.BRANCH);
  }
  for (const word of invalidReturns) {
    assert.equal(W.isRet(word), false, `0x${word.toString(16)} has authenticated RET with Rn != 31`);
    assert.notEqual(W.classifyWord(word), K.RET);
  }
  for (const word of [...invalidBranches, ...invalidReturns]) {
    const p = globalThis.AddressProvenance.create({ words: W });
    p.note(8, 0x1120n, 1);
    const kind = W.classifyWord(word);
    assert.equal(p.control(word, 0x1008n, kind).barrier, false,
      `0x${word.toString(16)} must not manufacture a control barrier`);
    assert.equal(p.base(8, 2, { allowEntryFallback: true }), 0x1120n);
  }
});
