...
test('overlapping exact tiers require semantic agreement and publish at most once', async () => {
  ...
  const blocking = new ScriptedExactBackend('blocking-wide', satOutcome, { busyMs: 25 });
  let starvedPublications = 0;
  const starvedResult = await new TieredBvBackend({ narrowBackend: quick, wideBackend: blocking }).createSession().check(candidate, { timeoutMs: 2 }).then((result) => { starvedPublications++; return result; });
  ...
  // The host deadline may expire after the first exact tier before the
  // replacement tier is entered; either path must publish only TIMEOUT once.
  assert.ok(blocking.calls <= 1);
