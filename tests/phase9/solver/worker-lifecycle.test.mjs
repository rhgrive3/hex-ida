...
test('canonical worker responses cannot omit the pending request token', async () => {
  class MissingTokenWorker extends MockWorker {
    dispatch(type, data) {
      const withoutToken = { ...data };
      delete withoutToken.token;
      return super.dispatch(type, withoutToken);
    }
  }
  const session = workerBackend(new MissingTokenWorker()).createSession();
  const result = await session.check(query('missing-canonical-token'));
  assert.equal(result.status, SOLVER_STATUS.PROVIDER_FAILURE);
