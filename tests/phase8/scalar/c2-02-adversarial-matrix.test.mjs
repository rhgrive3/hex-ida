...
test('vertical publication rejects a producer graph changed during descriptor capture', () => {
  const target = {
    id:1, kind:'arg', bits:8, signed:null, const:null, def:null, uses:[],
    origin:{ instructionIds:['instruction_vertical_dynamic_value'] },
  };
  let mutate = true;
  const value = new Proxy(target, {
    getOwnPropertyDescriptor(object, key) {
  ...
  const outcome = runPhase8Stage({
    ir:{
      values:[value], blocks:[{ id:'entry' }], entry:'entry',
      origin:{ instructionIds:['instruction_vertical_dynamic_function'] },
    },
    opts:{},
  }, { stages:['canonical-facts'] });
  assert.equal(outcome.ledger.published, false);
  assert.equal(outcome.ledger.stopReason, 'semantic-snapshot-changed-before-publication');
});

test('vertical publication rechecks a generic-first alias after post-seed mutation', () => {
  const target = {
    id:1, kind:'arg', bits:8, signed:null, const:null, def:null, uses:[],
    origin:{ instructionIds:['instruction_role_union_vertical_value'] },
  };
  const value = new Proxy(target, {
    ownKeys(object) { return Reflect.ownKeys(object).filter((key) => key !== 'bits'); },
  });
  ...
  const outcome = runPhase8Stage({
    ir:{
      a:value,
      values:[value], blocks:[{ id:'entry' }], entry:'entry',
      origin:{ instructionIds:['instruction_role_union_vertical_function'] },
    },
    opts:{},
  }, {
    stages:['canonical-facts'],
...
test('vertical publication binds retained immediate-dominance facts', () => {
  const ir = identityFixture('c2-02-idom-publication-binding');
  let checks = 0;
  const outcome = runPhase8Stage({ ir, opts:{} }, {
    stages:['canonical-facts'],
    shouldAbort() {
      checks += 1;
      if (checks === 1) ir.idom[0] = 0;
      return false;
    },
  });
  assert.equal(outcome.ledger.published, false);
...
test('the immediate-post-dominator alias is probed and must agree with ipdom', () => {
  ...
  const ipdomOnly = identityFixture('c2-02-ipdom-only-seed');
  delete ipdomOnly.idom;
  delete ipdomOnly.dominators;
  delete ipdomOnly.postDominators;
  const ipdomOnlyState = seedAnalysisState(ipdomOnly);
  assert.equal(ipdomOnlyState.version('dominators'), 1,
    'an ipdom-only upstream fact must still seed the dominance analysis');
  assert.deepEqual(ipdomOnlyState.get('dominators').ipdom, ipdomOnly.ipdom);
});
