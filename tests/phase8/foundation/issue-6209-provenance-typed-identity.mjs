...
assert.deepEqual(merged.ssaDefs, ['def-A']);
assert.equal(merged.ssaDefs.includes('def-A'), true);
...
assert.deepEqual(mergeSource({ ssaUses: ['use-A'] }, { ssaUses: [['use-A']] }).ssaUses, ['use-A']);
...
// 3. BigInt is canonical only for address provenance; non-address fields reject it.
...
// 4. Canonical merge/dedupe results are preserved, including primitive string IDs.
assert.deepEqual(mergeSource({ rows: [1] }, { rows: [1, 2] }).rows, [1, 2]);
assert.deepEqual(mergeSource({ addresses: [0x1000n] }, { addresses: [0x1000n] }).addresses, [0x1000n]);
assert.deepEqual(mergeSource({ ssaDefs: ['def-A'] }, { ssaDefs: ['def-A', 'def-B'] }).ssaDefs, ['def-A', 'def-B']);
assert.deepEqual(sourceOf({ ssaDef: 'def-A', ssaUse: 'use-A', irId: 'ir-A', row: 4, address: 0x1000n }).ssaDefs, ['def-A']);

// 5. Malformed metadata never collides with canonical numeric or string load identities.
...
const canonicalString = expr.load(location, 64, sourceOf({ ssaDefs: ['7'] }));
const structuredString = expr.load(location, 64, sourceOf({ ssaDefs: [['7']] }));
...
assert.notEqual(structuralKey(canonical), structuralKey(canonicalString));
assert.notEqual(structuralKey(canonicalString), structuralKey(structuredString));
...
assert.equal(structuralKey(canonicalString), structuralKey(expr.load(location, 64, sourceOf({ ssaDefs: ['7'] }))));
...
assert.match(structuralKey(expr.load(location, 64, sourceOf({ ssaDefs: ['def-A'] }))), /:ssa:s:"def-A"$/);
