...
function repairableResult(warnings) {
  ...
  phi.incoming.push({ from: 1, value: init }, { from: 0, value: updated });
...
test('#5135 loop repair cannot erase unrelated aggregate control-flow diagnostics', () => {
  const unrelated = '2 control-flow edge(s) remain explicit because a safe source structure was not proven.';
  const other = 'Semantic IR budget truncated this function; the result is partial.';
  const { result, blockAddress } = repairableResult([unrelated, other]);

  repairCanonicalPostTestLoop(result, blockAddress);

  assert.equal(result.ctx.loopRepair, 'ir-def-use', 'fixture must exercise the successful repair path');
  assert.ok(result.lines.some((line) => line.text === 'do {'));
  assert.deepEqual(result.warnings, [unrelated, other],
    'string-only aggregate warnings have no edge identity and must be retained fail-closed');
});
