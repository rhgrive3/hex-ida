...
// #5567: the post-test loop repair must only rewrite loops whose compared value
// has loop-carried state (a PHI or an in-loop redefinition). A `constant + const`
// self-loop describes a fixed expression and never terminates; converting it to
// `do { i++ } while (i < n)` fabricates a finite loop out of an infinite one.

function constantSelfLoopFixture() {
  const zero = { id: 'zero', bits: 64, const: 0n };
  const one = { id: 'one', bits: 64, const: 1n };
  const limit = { id: 'limit', bits: 64, const: 10n };
  const updated = { id: 'u', bits: 64, const: null, def: { op: 'bin', sub: 'add', args: [{ value: zero }, { value: one }] } };
...
function phiRecurrenceFixture() {
  ...
  const phi = { op: 'phi', incoming: [{ from: 1, value: zero }, { from: 0, value: updated }] };
  prior.def = phi;
  const flags = { id: 'flags', bits: 4, def: { op: 'cmp', sub: 'sub', bits: 64, args: [{ value: updated }, { value: limit }] } };
  const term = { op: 'cbr', row: 1, address: 0x1004n, cond: 'lt', extra: { target: 0x1000n }, args: [{ value: flags }] };
  const loop = { header: 0, nodes: new Set([0]), exits: new Set([1]) };
...
test('#5567: constant compared operand without loop-carried state is not rewritten', () => {
  const { result, blockAddress } = constantSelfLoopFixture();
...
test('#5567: exact PHI recurrence remains eligible for post-test repair', () => {
  const { result, blockAddress } = phiRecurrenceFixture();
  repairCanonicalPostTestLoop(result, blockAddress);
