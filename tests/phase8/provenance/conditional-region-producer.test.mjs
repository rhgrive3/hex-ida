...
function render(kind = 'diamond', options = {}) {
  ...
  if (kind === 'plain') {
  ...
  } else if (kind === 'terminal-if-else') {
    f.conditionalBranch(condition, 1, 2);
    f.block(1); f.ret();
    f.block(2); f.ret();
  } else if (kind === 'terminal-loop') {
    f.conditionalBranch(condition, 1, 2);
    f.block(1); f.ret();
    ...
    f.block(4); f.store(f.constant(4n, 32)); f.ret();
    f.block(5); f.ret();
  } else if (kind === 'cyclic-shared-cleanup') {
    f.conditionalBranch(condition, 1, 2);
    f.block(1); f.store(f.constant(1n, 32)); f.branch(4);
    f.block(2); f.conditionalBranch(condition, 5, 3);
    f.block(3); f.store(f.constant(3n, 32)); f.branch(4);
    f.block(4); f.conditionalBranch(condition, 6, 4);
    f.block(5); f.ret();
    f.block(6); f.ret();
  } else {
    f.conditionalBranch(condition, 1, 2);
...
test('terminal and early-exit goto reduction stays proof-gated', () => {
  const terminal = render('terminal-if-else').seed;
  assert.equal(terminal.coverage.mode, 'structured');
  assert.equal(terminal.coverage.structuredMissing, 0);
  assert.match(terminal.pseudocode, /if \(/);
  assert.match(terminal.pseudocode, /} else {/);
  assert.doesNotMatch(terminal.pseudocode, /goto loc_/);

  const nonTerminating = render('terminal-loop').seed;
  assert.equal(nonTerminating.coverage.mode, 'linear');
  assert.match(nonTerminating.pseudocode, /goto loc_/);
