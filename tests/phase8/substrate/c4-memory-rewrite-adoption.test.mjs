...
function passFor(validation,rewrite,kind='memory-byte-rewrite'){
  const d=descriptor();
  return {descriptor:d,run(){return createValidatedPassResult({descriptor:d,status:'changed',changed:true,transforms:[{kind,targets:['memory-state'],proof:'C4-04B bounded semantic equivalence',rewrite,validation}]});}};
}
...
test('C4-04B canonical terminal fault effects can authorize only their declared effect rewrite',async()=>{
  ...
  assert.equal(validation.validation,'equivalent',validation.reason);
  const outcome=runPassTransaction(createAnalysisState(STATE),passFor(validation,proof,'terminal-control-effect-rewrite'),{},{});
  assert.equal(outcome.committed,true,outcome.stopReason);
});
