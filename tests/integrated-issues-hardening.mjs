...
{
  const { parseDebugInfo } = await import('../js/analysis/debug/dwarf.js');
  ...
  // Offset past the end of .debug_str: previously an empty '' on a complete DIE.
  {
    const result = parse([0], [4, 0, 0, 0]);   // length 1, offset 4 is out of bounds
  ...
  // No NUL before the end of the section: previously the whole span decoded.
  {
  ...
  // A terminated string still parses and stays complete.
  {
  ...
  console.log('  ok #1861 DWARF strp references are bounds-checked and terminated');
}
