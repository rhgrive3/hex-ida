...
- [X] T017 Execute `/speckit.converge`, append any unmet work to `specs/001-loaded-pointer-recovery/tasks.md`, and repeat implement/converge until CONVERGED — run 2026-09-01 on current main `c78e1b98`/branch head `d2574c3e`: FR-001–FR-012 and SC-001–SC-007 verified present and green on current main (focused 11/11 twice, pointsto 45/45, Phase 7 85/85, ownership valid); no new actionable findings, no tasks appended (CONVERGED)
- [X] T018 Refetch live main, recheck open-PR/Issue-Agent collision against the actual file inventory, reconcile once through Sol, and record the new base relationship in `docs/analysis-improvement-finding-ledger.md` — refetched 2026-09-01: main `c78e1b98` (PR #3284 head), branch `feat/analysis-partial-closure-20260901` is one generated-output-resync commit `d2574c3e` ahead, merge base `c78e1b98`, zero conflicts (merge-tree `e980aba2`)
- [X] T019 Regenerate `userscript/hex.user.template.js` and `userscript/release-version.json` only with the canonical builder, then prove a second build has zero diff — canonical `npm run userscript:build` produced serial `2322242129`, release identity `11d5a7aca81fd31d4e3c94dd0a935a681ddb63bc6fd34896d9428931ff2a9150`, buildId `0303fc9a02c2ceb90b1c78a5`; second and third builds byte-identical (zero diff); committed as `d2574c3e` (main's committed output was stale relative to post-`f205d17b` runtime source)
- [X] T020 Run the Phase 7 verifier against the exact branch head and record source SHA, verifier/runtime identity, corpus identity, and result in `docs/analysis-improvement-finding-ledger.md` — `P7_VERDICT=READY` at exact head `d2574c3e2f928f59790cf2f156d92ace365f2f15` (tree `e980aba2`, clean), verifier phase7.verifier 1.0.0 source sha256 `1de28d218aa3eda0`, corpus `phase7-alias-memory-corpus v1` digest `519bd15f3a918dbc2b436aca4870455d` frozen match true, false NoAlias/MustAlias = 0
- [X] T021 Build the actual candidate merge tree against freshly fetched main, run applicable owned gates on that exact tree, and record its SHA/results in `docs/analysis-improvement-finding-ledger.md` — candidate tree `e980aba2508fd18ad5e2b1397c8dab1b7342459c` (fast-forward over main `c78e1b98`, 0 conflicts); Phase 7 verifier READY, ownership manifest valid, focused 11/11, pointsto 45/45 all green on this exact tree
- [ ] T022 Complete Sol final diff/soundness review, required GitHub CI, expected-head merge of PR evidence, post-merge live-main verification, and final C1-01 `MERGED` entry in `docs/analysis-improvement-finding-ledger.md` — remaining: push branch/PR + GitHub exact-head CI + expected-head merge; all local closure evidence is complete on `d2574c3e`
