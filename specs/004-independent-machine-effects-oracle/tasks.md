...
- [ ] T037 Run the permanent exact-head verifier against a clean final head with current external evidence; local schema fixtures do not satisfy this task
- [ ] T038 Build and validate the actual candidate merge tree through the Sol integration lane; no historical candidate-tree identity is current evidence
- [ ] T039 Reconcile moving `main`, canonical integration-owned generated output, required CI, and post-merge live-main verification through Sol

Open production dependencies are tracked separately from the completed oracle-schema tasks above:
the real x86 producer path (review item T017) and current-main `apply_damage` dependency (review
item T018) remain open. This oracle-only lane does not edit or close either item.
