...
For T011–T017 and every applicable T026–T036/T045/T051+ component, the `owned`
...
- [ ] T046 [CAMP] Complete the mandatory master-phase pre-fanout gate and record it in `specs/005-analysis-final-closure/evidence/pre-fanout.md`
  - **Contract** — Objective: satisfy Guardrails §3.1 before any T011–T017 component implementation begins. Current evidence: living branch/PR #6429 exists; machine ownership, exact inventory, frozen target/invalidation contracts, recursive runner, local walking skeleton/lint, and the initial local review are historical evidence only. The independent Sol adversarial review found fail-open blockers in checkpoint lineage, mutable task handoff anchoring, recovery-ref fetch isolation, and path decoding; executable/documentation repairs are now present, but a fresh independent Sol review on the exact repaired tree, the final focused suite, and the hosted exact-head/base run remain pending. Owner/model: SOL Ultra. Risk: RELEASE. Dependencies: T009-T010. Owned paths: one campaign PR, `.github/workflows/final-closure-preflight.yml`, `tools/validation/final-closure/preflight.mjs`, `tests/final-closure/{preflight.test,run}.mjs`, the `package.json` canonical test hook, `contracts/{task-ownership,integration-inventory,closure-ledger,performance-locks}.json` or `.md` as applicable, the requirements checklist, data model, recovery and Spec Kit audit evidence, plan, quickstart, research, this task ledger, and this evidence file; `tests/phase4/walking-skeleton.test.mjs` is an unchanged read-only production-chain gate. Delta: add permanent event-authoritative exact-head and component candidate-merge-tree invocation, recursive growth-safe canonical test discovery, completed-dependency enforcement, machine-readable per-task allowlists and safe argv candidate-gate registry/execution, cumulative exact base-to-candidate inventory ownership, frozen performance/platform/global ownership identities, and machine-checked rolling checkpoints with the immutable four-commit transaction `I_i → M_i → G_i → E_i` (`M_i` is the exact two-parent candidate merge of `I_i` and `C_i`, `G_i` is the exact generated/reconciled product child of `M_i`, and `E_i` is the evidence-only child of `G_i`). The contract also requires exact `acceptedTaskId`/`taskHandoffs` head-and-tree binding, a canonical first-DONE T046 handoff anchor derived from immutable history, content-derived verifier/oracle blob evidence, two canonical generator runs with zero second diff on exact `G_i`, runtime replay of every rolling and independent-shadow gate against exact `G_i`, scratch-ref-only recovery fetches with unrelated-ref transaction protection, and a machine-checked Stage-A-to-Stage-B transition without weakening existing candidate-tree checks; synchronize the affected Spec Kit evidence with the corrected executable contract and current Graft authorization. Negative counterexample: component fanout before PR creation, verifier accepts a different SHA/base/ref, component tests run on an unmerged head, any owned/rolling/shadow gate is absent, shell-interpreted, skipped, or runs outside the synthetic candidate, an owned nested test is undiscovered or breaks the runner, DONE precedes a pending dependency, Stage B lacks accepted Stage A ancestry/evidence or reuses its worktree, a frozen threshold/source/global rule drifts, a changed path escapes the exact inventory/owner allowlist, concurrently unordered owners overlap, mock-only skeleton substitutes for production, desktop substitutes for iPad, `M_i` has the wrong parents/tree, `G_i` is absent or not the generated child of `M_i`, `E_i` is not evidence-only, `acceptedTaskId` attests another task's handoff, the mutable current inventory rewrites the canonical T046 handoff, verifier/oracle hashes are arbitrary or self-certifying, generator/replay runs on a different head/tree, recovery fetch rewrites the canonical recovery tracking ref or unrelated refs, invalid UTF-8/BOM/control bytes are silently normalized into an allowlisted path, or evidence is reused after an invalidating identity change. Tests: exact event/head/base/ref mutation, component merge-tree/out-of-lane mutation and three-kind gate execution, runner-growth/execution sentinels, completed-dependency ordering, Stage-B missing/stale/reused-worktree evidence, frozen performance/source/global rule mutations, expected/actual changed-path union, all-task ownership overlap, real production walking skeleton, target-proof schema, moving-main invalidation, exact `I_i → M_i → G_i → E_i` parent/tree lineage, task-handoff/immutable-anchor mutation, content-derived verifier/oracle blob mutation, exact-`G_i` generator-twice and runtime rolling/shadow replay, scratch recovery-ref/fetch transaction mutation, and invalid UTF-8/BOM path rejection. Integration test: canonical runner through both `npm test` and the dedicated integration/component preflight workflow, with the registry-selected owned, rolling-product, and independent-shadow gates on the exact candidate merge tree and unchanged Phase 4 production walking skeleton. Completion evidence: PR/base/head/tree/merge-tree IDs, exact verifier invocation, recursive runner/discovery proof, dependency/ownership/candidate-gate registry regressions and registry digest, exact cumulative changed-path inventory, immutable recovery-source and canonical T046 handoff proof, `I_i → M_i → G_i → E_i` lineage, content-derived generation/verifier/oracle evidence, exact-`G_i` generator-twice/runtime replay proof, scratch-ref/unrelated-ref protection, path-decoding rejection, walking-skeleton production path, target-device contract, reconciliation owner, invalidation set, independent fresh re-audit, and `PREFLIGHT_GREEN`. Status: PENDING.

- [ ] T049 [CAMP] Operate the Stage A rolling integration checkpoint transaction after every accepted recovery component
  - **Contract** — Objective: enforce Guardrails §3.3/§3.4 between every T011–T017 component merge while keeping component owners out of shared/generated output. Current evidence: T046 is not green and no component is authorized. Owner/model: SOL Ultra living integration/generated-output owner. Risk: RELEASE. Dependencies: T046. Owned paths: cumulative integration inventory, campaign task/checkpoint evidence, and integration-owned generated userscript/deployment identity outputs only; component production/test paths remain read-only during the transaction. Delta: for each completed component handoff, prove and accept its exact candidate merge tree, refresh the exact cumulative inventory, reconcile shared contracts, regenerate canonically twice with zero second diff, run rolling product plus independent shadow/verifier gates, and record the exact checkpoint before another merge. Negative counterexample: a second component merges before the prior generated/checkpoint/verifier transaction is green, a component worker commits generated output, a handoff SHA/tree is stale, or cumulative inventory omits a merged path. Tests: exact component/integration changed-file union, handoff identity, canonical generation twice, rolling vertical gate, independent shadow verifier, and next-component lock rejection. Integration test: one machine-readable exact checkpoint for every accepted T011–T017 lane. Completion evidence: monotonically ordered checkpoint records covering every accepted recovery component, with all applicable T011–T017 tasks complete and no unlocked merge interval. Status: PENDING.
...
- [ ] T018 [US1] Revalidate all ten recovery rows and close the superseded C2 row in `specs/005-analysis-final-closure/evidence/recovery-matrix.md`
  - **Contract** — Objective: make handoff unfinished count zero after every component has passed the T049 rolling checkpoint transaction. Current evidence: C2 unknown is superseded by `f1dbad86`; other rows await T011-T017. Owner/model: SOL Ultra. Risk: RELEASE. Dependencies: T011-T017 and T049. Owned paths: recovery matrix and campaign tasks only. Delta: evidence-backed status updates. Negative counterexample: composite REC-P8-S marked done despite conflict or a component marked terminal without its exact T049 checkpoint. Tests: every row's focused exit gate, checkpoint coverage, and `memoryssa-unknown-partition`. Integration test: affected subsystem matrix. Completion evidence: ten terminal rows, PARTIAL/CONFLICTED/NOT STARTED zero, and exact T049 coverage for every accepted component. Status: PENDING.
...
- T049 is the Stage A living integration checkpoint owner: after each accepted
  T011-T017 lane it must finish the §3.4 transaction before another component
