...
T029/T031: pipeline now exposes independently proved rewriting and consumes branded e-graph proposals only with a proof bound to the current AST pair, SSA/provenance and context. Root review found and fixed wrong-pair token replay and mutation during solver await; the latter now binds the pre-translation pair and rechecks before minting. Focused T029 tests 5/5 pass, prior combined owner batch 46/46 and bounded e-graph tests 4/4 pass. T030 remains incomplete: actual exception-region transforms and production consumption are in progress; the earlier edge-accounting/budget improvements alone do not close the task.
