...
2. **Given** a required check, review finding, generated artifact, or runtime proof that is red, missing, or unexplained, **When** merge readiness is evaluated, **Then** the merge remains blocked.
...
- **FR-006**: Every required recovery-integration check MUST be present, completed, and successful; failure explanations record a blocked state and never convert a required red check into acceptance. The candidate MUST also have zero actionable review findings, canonical generated artifacts, and exact candidate-head plus candidate-merge-tree evidence.
...
- **FR-013**: The campaign MUST record exactly one mandatory terminal evidence record for each of `falseExactNoAlias`, `falseExactMustAlias`, `falseExactIndirectTarget`, `falseExactType`, `semanticMismatch`, `stalePublicationAfterCancel`, and `invalidWriterOutputAccepted`. Every record MUST have a positive denominator and observed sample count, a zero observed value, and exact command, corpus, fixture-set, and candidate identities; missing, nonterminal, or zero-denominator records cannot satisfy the requirement.
...
- **SC-002**: The recovery candidate is present in live main, every required check is present, completed, and successful on the exact accepted product, and actionable review findings equal zero.
...
- **SC-004**: `falseExactNoAlias`, `falseExactMustAlias`, `falseExactIndirectTarget`, `falseExactType`, `semanticMismatch`, `stalePublicationAfterCancel`, and `invalidWriterOutputAccepted` each have exactly one mandatory terminal record with a positive denominator and observed sample count and are zero on every required command-, fixture-, corpus-, and candidate-identity-bound denominator.
