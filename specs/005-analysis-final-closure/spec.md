...
- **FR-006**: Recovery integration MUST have zero unexplained required test failures, zero actionable review findings, canonical generated artifacts, and exact candidate-head plus candidate-merge-tree evidence.
...
- **FR-013**: The campaign MUST record separate zero counters for `falseExactNoAlias`, `falseExactMustAlias`, `falseExactIndirectTarget`, `falseExactType`, `semanticMismatch`, `stalePublicationAfterCancel`, and `invalidWriterOutputAccepted`, each bound to its command, corpus identity, denominator, and candidate identity.
...
- **SC-002**: The recovery candidate is present in live main, with zero unexplained required failures and zero actionable review findings on the exact accepted product.
...
- **SC-004**: `falseExactNoAlias`, `falseExactMustAlias`, `falseExactIndirectTarget`, `falseExactType`, `semanticMismatch`, `stalePublicationAfterCancel`, and `invalidWriterOutputAccepted` are each zero on every required identity-bound locked corpus and denominator.
