...
try:
 ...
 report['afterLocalRoutesHashes']=hashes()
 assert all(report['afterLocalRoutesHashes'][p]==second[p] for p in generated[:2]),'loader or release changed during local routes'
 # wrangler.jsonc explicitly invokes write-deployment-identity.mjs after its
 # build. That stamp is a different stage, not a nondeterministic loader.
 stamp=Path(generated[2]).read_text()
 expectedStamp='// Generated by scripts/write-deployment-identity.mjs during the Workers build.\nexport const DEPLOYMENT_COMMIT=null;\n'
 assert not os.environ.get('WORKERS_CI_COMMIT_SHA'),'this check is local-only'
 assert stamp==expectedStamp,'local ephemeral projection must not attest a deployment'
 report['localRouteStampNull']=True;save()
 run('canonical-after-local-routes',['npm','run','userscript:build'],180)
 assert hashes()==second,'canonical projection did not return to the same three hashes'
 report['canonicalProjectionRestored']=True
